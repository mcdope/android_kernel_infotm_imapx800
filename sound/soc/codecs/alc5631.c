...
EXPORT_SYMBOL(rt5631_codec);
int rt5631_switch_spk(int dir);

int alc5631_play(void)                                             
{                                                                  
    int iface = 0;                                                 
    ...
    snd_soc_update_bits(rt5631_codec, RT5631_PWR_MANAG_ADD3,       
	    ...
	    RT5631_PWR_FAST_VREF_CTRL);                            
    snd_soc_write(rt5631_codec, RT5631_PWR_MANAG_ADD1, 0x93e0);    
    snd_soc_write(rt5631_codec, RT5631_PWR_MANAG_ADD2, 0xf028);    
    snd_soc_write(rt5631_codec, RT5631_PWR_MANAG_ADD3, 0xe01e);    
    snd_soc_write(rt5631_codec, RT5631_PWR_MANAG_ADD4, 0xcc00);    

    rt5631_switch_spk(1);                                          
    iface |= RT5631_SDP_MODE_SEL_SLAVE;                            
    snd_soc_write(rt5631_codec, RT5631_SDP_CTRL, iface);           
    snd_soc_update_bits(rt5631_codec, RT5631_SDP_CTRL,             
	    RT5631_SDP_I2S_DL_MASK, 0);                            
    snd_soc_update_bits(rt5631_codec, RT5631_STEREO_AD_DA_CLK_CTRL,
	    RT5631_I2S_PRE_DIV_MASK |                              
	    RT5631_I2S_LRCK_SEL_N_BCLK_MASK |                      
	    RT5631_ADDA_FILTER_CLK_SEL_MASK,                       
	    0x1000);                                               

    //rt5631_read_register(rt5631_codec);                            
    return 0;                                                      
}                                                                  
EXPORT_SYMBOL(alc5631_play);                                       
...
static void DoItemCfgOri(ItemAudioCfg_T *p)
{
	...
	if(item_exist("codec.micvolume"))
    {
        p->Mic1Volume=item_integer("codec.micvolume", 0);
		printk("default volume:%d\n",p->Mic1Volume);	
    }
    else
	{
...
if(p->SpkVolume >=0 && p->SpkVolume <=100 ){

	val=(3900-(39*p->SpkVolume))/100;
	printk("default spk volume %x\n",val);
	snd_soc_update_bits(codec, RT5631_SPK_OUT_VOL,0x3f<<RT5631_L_VOL_SHIFT,val<<RT5631_L_VOL_SHIFT);
	snd_soc_update_bits(codec, RT5631_SPK_OUT_VOL,0x3f<<RT5631_R_VOL_SHIFT,val<<RT5631_R_VOL_SHIFT);
}

if(p->Mic1Volume >=0 && p->Mic1Volume <=100 ){

	val=(p->Mic1Volume)/12;
	printk("default spk mic1volume %x\n",val);
	snd_soc_update_bits(codec, RT5631_MIC_CTRL_2,0xf<<RT5631_M_MIC1_OUTMIXR_BIT,val<<RT5631_M_MIC1_OUTMIXR_BIT);
}
...
static int rt5631_reg_init(struct snd_soc_codec *codec)
{
	...
	DoItemHALSetVolume(codec,&AudioItemCfgObj);
	return 0;
}
...
struct rt5631_priv {
	...
	struct work_struct SPKswitchWork;
	int SPKswitchVal;
};
		...
		unsigned int reg, unsigned int value)
{
	...
	if (ret < 0) {
		dev_err(codec->dev, "Write: Failed to set private addr: %d\n", ret);
		goto err;
	}
	...
	struct snd_soc_codec *codec, unsigned int reg)
{
	...
	ret = snd_soc_write(codec, RT5631_INDEX_ADD, reg);
	if (ret < 0) {
		dev_err(codec->dev, "Read: Failed to set private addr: %d\n", ret);
		return ret;
	}
...
static int rt5631_volatile_register(struct snd_soc_codec *codec, unsigned int reg)
{
    return 1;
	switch (reg) {
	case RT5631_RESET:
...
#ifdef ALC5631_DEBUG
static int rt5631_read_register(struct snd_soc_codec *codec)
{
	printk("RT5631_RESET: 0x%x\n", snd_soc_read(codec, RT5631_RESET));
	printk("RT5631_SPK_OUT_VOL: 0x%x\n", snd_soc_read(codec, RT5631_SPK_OUT_VOL));
	printk("RT5631_HP_OUT_VOL: 0x%x\n", snd_soc_read(codec, RT5631_HP_OUT_VOL));
	printk("RT5631_MONO_AXO_1_2_VOL: 0x%x\n", snd_soc_read(codec, RT5631_MONO_AXO_1_2_VOL));
	printk("RT5631_AUX_IN_VOL: 0x%x\n", snd_soc_read(codec, RT5631_AUX_IN_VOL));
	printk("RT5631_STEREO_DAC_VOL_1: 0x%x\n", snd_soc_read(codec, RT5631_STEREO_DAC_VOL_1));
	printk("RT5631_MIC_CTRL_1: 0x%x\n", snd_soc_read(codec, RT5631_MIC_CTRL_1));
	printk("RT5631_STEREO_DAC_VOL_2: 0x%x\n", snd_soc_read(codec, RT5631_STEREO_DAC_VOL_2));
	printk("RT5631_ADC_CTRL_1: 0x%x\n", snd_soc_read(codec, RT5631_ADC_CTRL_1));
	printk("RT5631_ADC_REC_MIXER: 0x%x\n", snd_soc_read(codec, RT5631_ADC_REC_MIXER));
	printk("RT5631_ADC_CTRL_2: 0x%x\n", snd_soc_read(codec, RT5631_ADC_CTRL_2));
	printk("RT5631_VDAC_DIG_VOL: 0x%x\n", snd_soc_read(codec, RT5631_VDAC_DIG_VOL));
	printk("RT5631_OUTMIXER_L_CTRL: 0x%x\n", snd_soc_read(codec, RT5631_OUTMIXER_L_CTRL));
	printk("RT5631_OUTMIXER_R_CTRL: 0x%x\n", snd_soc_read(codec, RT5631_OUTMIXER_R_CTRL));
	printk("RT5631_AXO1MIXER_CTRL: 0x%x\n", snd_soc_read(codec, RT5631_AXO1MIXER_CTRL));
	printk("RT5631_AXO2MIXER_CTRL: 0x%x\n", snd_soc_read(codec, RT5631_AXO2MIXER_CTRL));
	printk("RT5631_MIC_CTRL_2: 0x%x\n", snd_soc_read(codec, RT5631_MIC_CTRL_2));
	printk("RT5631_DIG_MIC_CTRL: 0x%x\n", snd_soc_read(codec, RT5631_DIG_MIC_CTRL));
	printk("RT5631_MONO_INPUT_VOL: 0x%x\n", snd_soc_read(codec, RT5631_MONO_INPUT_VOL));
	printk("RT5631_SPK_MIXER_CTRL: 0x%x\n", snd_soc_read(codec, RT5631_SPK_MIXER_CTRL));
	printk("RT5631_SPK_MONO_OUT_CTRL: 0x%x\n", snd_soc_read(codec, RT5631_SPK_MONO_OUT_CTRL));
	printk("RT5631_SPK_MONO_HP_OUT_CTRL: 0x%x\n", snd_soc_read(codec, RT5631_SPK_MONO_HP_OUT_CTRL));
	printk("RT5631_SDP_CTRL: 0x%x\n", snd_soc_read(codec, RT5631_SDP_CTRL));
	printk("RT5631_MONO_SDP_CTRL: 0x%x\n", snd_soc_read(codec, RT5631_MONO_SDP_CTRL));
	printk("RT5631_STEREO_AD_DA_CLK_CTRL: 0x%x\n", snd_soc_read(codec, RT5631_STEREO_AD_DA_CLK_CTRL));
	printk("RT5631_PWR_MANAG_ADD1: 0x%x\n", snd_soc_read(codec, RT5631_PWR_MANAG_ADD1));
	printk("RT5631_PWR_MANAG_ADD2: 0x%x\n", snd_soc_read(codec, RT5631_PWR_MANAG_ADD2));
	printk("RT5631_PWR_MANAG_ADD3: 0x%x\n", snd_soc_read(codec, RT5631_PWR_MANAG_ADD3));
	printk("RT5631_PWR_MANAG_ADD4: 0x%x\n", snd_soc_read(codec, RT5631_PWR_MANAG_ADD4));
	printk("RT5631_GEN_PUR_CTRL_REG: 0x%x\n", snd_soc_read(codec, RT5631_GEN_PUR_CTRL_REG));
	printk("RT5631_GLOBAL_CLK_CTRL: 0x%x\n", snd_soc_read(codec, RT5631_GLOBAL_CLK_CTRL));
	printk("RT5631_PLL_CTRL: 0x%x\n", snd_soc_read(codec, RT5631_PLL_CTRL));
	printk("RT5631_INT_ST_IRQ_CTRL_1: 0x%x\n", snd_soc_read(codec, RT5631_INT_ST_IRQ_CTRL_1));
	printk("RT5631_INT_ST_IRQ_CTRL_2: 0x%x\n", snd_soc_read(codec, RT5631_INT_ST_IRQ_CTRL_2));
	printk("RT5631_GPIO_CTRL: 0x%x\n", snd_soc_read(codec, RT5631_GPIO_CTRL));
	printk("RT5631_MISC_CTRL: 0x%x\n", snd_soc_read(codec, RT5631_MISC_CTRL));
	printk("RT5631_DEPOP_FUN_CTRL_1: 0x%x\n", snd_soc_read(codec, RT5631_DEPOP_FUN_CTRL_1));
	printk("RT5631_DEPOP_FUN_CTRL_2: 0x%x\n", snd_soc_read(codec, RT5631_DEPOP_FUN_CTRL_2));
	printk("RT5631_JACK_DET_CTRL: 0x%x\n", snd_soc_read(codec, RT5631_JACK_DET_CTRL));
	printk("RT5631_SOFT_VOL_CTRL: 0x%x\n", snd_soc_read(codec, RT5631_SOFT_VOL_CTRL));
	printk("RT5631_ALC_CTRL_1: 0x%x\n", snd_soc_read(codec, RT5631_ALC_CTRL_1));
	printk("RT5631_ALC_CTRL_2: 0x%x\n", snd_soc_read(codec, RT5631_ALC_CTRL_2));
	printk("RT5631_ALC_CTRL_3: 0x%x\n", snd_soc_read(codec, RT5631_ALC_CTRL_3));
	printk("RT5631_PSEUDO_SPATL_CTRL: 0x%x\n", snd_soc_read(codec, RT5631_PSEUDO_SPATL_CTRL));
	printk("RT5631_INDEX_ADD: 0x%x\n", snd_soc_read(codec, RT5631_INDEX_ADD));
	printk("RT5631_INDEX_DATA: 0x%x\n", snd_soc_read(codec, RT5631_INDEX_DATA));
	printk("RT5631_EQ_CTRL: 0x%x\n", snd_soc_read(codec, RT5631_EQ_CTRL));
	printk("RT5631_VENDOR_ID: 0x%x\n", snd_soc_read(codec, RT5631_VENDOR_ID));
	printk("RT5631_VENDOR_ID1: 0x%x\n", snd_soc_read(codec, RT5631_VENDOR_ID1));
	printk("RT5631_VENDOR_ID2: 0x%x\n", snd_soc_read(codec, RT5631_VENDOR_ID2));
}
#endif
static int rt5631_readable_register(struct snd_soc_codec *codec, unsigned int reg)
{
	switch (reg) {
	case RT5631_RESET:
...
#if 1//Fix recording fail
static void rt5631_cap_work(struct work_struct *work)
{
 	struct snd_soc_codec *codec = rt5631_codec;
	int ret = 0;
	printk("%s(%d) -- ADC Power On!!!\n", __func__, __LINE__);
	ret = snd_soc_update_bits(codec, RT5631_PWR_MANAG_ADD1, RT5631_PWR_ADC_L_CLK | RT5631_PWR_ADC_R_CLK,
		RT5631_PWR_ADC_L_CLK | RT5631_PWR_ADC_R_CLK);
	printk("rt5631_cap_work..... is %d\n", ret);
    mdelay(250);
	return;
}
static DECLARE_DELAYED_WORK(rt5631_delay_cap, rt5631_cap_work);
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMD:
		printk("%s(%d) -- SND_SOC_DAPM_POST_PMD\n", __func__, __LINE__);
		snd_soc_update_bits(codec, RT5631_PWR_MANAG_ADD1, RT5631_PWR_ADC_L_CLK | RT5631_PWR_ADC_R_CLK, 0);
		break;

	case SND_SOC_DAPM_POST_PMU:
		printk("%s(%d) -- SND_SOC_DAPM_POST_PMU\n", __func__, __LINE__);
		schedule_delayed_work(&rt5631_delay_cap, msecs_to_jiffies(500));//delay time can be adjusted to enable ADC power
		break;
...
#else //Fix recording fail
	SND_SOC_DAPM_ADC_E("Left ADC", "HIFI Capture", SND_SOC_NOPM, 0, 0,
		adc_event, SND_SOC_DAPM_PRE_PMD | SND_SOC_DAPM_POST_PMU),
	SND_SOC_DAPM_ADC("Right ADC", "HIFI Capture", SND_SOC_NOPM, 0, 0),
#endif
...
EXPORT_SYMBOL(rt5631_switch_spk);

#define LOG printk
int rt5631_spk_switch_Delay_mute(int dir)
{
	struct rt5631_priv *rt5631 =0;

	if(NULL!=rt5631_codec)
	{
		LOG("[%s] %d\n",__FUNCTION__,dir);
		rt5631= snd_soc_codec_get_drvdata( rt5631_codec);
		rt5631->SPKswitchVal=dir;
		schedule_work(&rt5631->SPKswitchWork);
	}

}
EXPORT_SYMBOL(rt5631_spk_switch_Delay_mute);

static void rt5631_mute(struct snd_soc_codec *codec, int dir)
{
    if (dir)
        snd_soc_update_bits(codec, RT5631_SPK_OUT_VOL,
                RT5631_L_MUTE | RT5631_R_MUTE,0);
...
int rt5631_flag = 0;
EXPORT_SYMBOL(rt5631_flag);

static int rt5631_switch_mute(struct snd_soc_dai *dai, int mute)
{
    struct snd_soc_codec *codec = dai->codec;
...
static void rt5631_gpio_jack_det_init(struct snd_soc_codec *codec)
{
        ...
        snd_soc_update_bits(codec,RT5631_INT_ST_IRQ_CTRL_1,
                RT5631_ENABLE_JACK_DET_STATUS,
                RT5631_ENABLE_JACK_DET_STATUS);
		if(item_exist("codec.hp_highlevel_en") && item_integer("codec.hp_highlevel_en", 0)) {
	        snd_soc_update_bits(codec,RT5631_JACK_DET_CTRL,
	                RT5631_JD_USE_MASK|
...
static int rt5631_code_init(struct snd_soc_codec *codec)
{
    struct rt5631_priv *rt5631 = snd_soc_codec_get_drvdata(codec);
    int val;
    ...
    rt5631_gpio_jack_det_init(codec);
...
static int rt5631_probe(struct snd_soc_codec *codec)
{
	...
	DoItemCfgOri(&AudioItemCfgObj);	
    rt5631_code_init(codec);
    ...
    snd_soc_write(codec, 0x48, 0xf428);//jason & chuanjin 2012-10-10 for nanmo request : disable protect function,[nanmo MID 0.5% speaker  NG  issue]
...
struct snd_soc_dai_ops rt5631_ops = {
    ...
    .digital_mute = rt5631_switch_mute,
};
...
static void SPK_switch_work(struct work_struct *work)
{
struct rt5631_priv *rt5631 = container_of(work, struct rt5631_priv , SPKswitchWork);

rt5631_switch_spk(rt5631->SPKswitchVal);
		    ...
		    const struct i2c_device_id *id)
{
	...
	if (NULL == rt5631) {
        printk("mem alloc for rt5631 failed\n");
		return -ENOMEM;
    }
    ...
    printk("buf is %s\n", buf);
#if 0
    rt5631->regulator = regulator_get(&i2c->dev, buf);
    if (IS_ERR(rt5631->regulator)) {
        printk("%s: get regulator fail\n", __func__);
        return -1;
    }
    ret = regulator_set_voltage(rt5631->regulator, 3300000, 3300000);
    if(ret) {
        printk("%s: set regulator fail\n", __func__);
        return -1;
    }

    regulator_enable(rt5631->regulator);
    printk("%s: regulator enable\n", __func__);
#endif
	INIT_WORK(&rt5631->SPKswitchWork, SPK_switch_work);
    ...
    ret = snd_soc_register_codec(&i2c->dev,
            &soc_codec_dev_rt5631, rt5631_dai, ARRAY_SIZE(rt5631_dai));
    if (ret < 0) {
        printk("register rt5631 codec err: %d\n", ret);
        return ret;
    }
	return 0;
}
...
static __devexit int rt5631_i2c_remove(struct i2c_client *client)
{
	...
	//regulator_put(rt5631->regulator);
...
EXPORT_SYMBOL(rt5631_i2c_driver);
