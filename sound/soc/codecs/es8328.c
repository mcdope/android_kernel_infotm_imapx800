...
int AngVal,DigtVal;
	...
	snd_soc_write(codec, ES8328_DACCONTROL24,(unsigned int) AngVal); //0x2e
	dbg("ES8328 %x\n", snd_soc_read(codec, ES8328_DACCONTROL24));
	snd_soc_write(codec, ES8328_DACCONTROL25, (unsigned int)AngVal); //0x2f
	dbg("ES8328 %x\n", snd_soc_read(codec, ES8328_DACCONTROL25));

	snd_soc_write(codec, ES8328_DACCONTROL4, (unsigned int)DigtVal); //0x1a
	dbg("ES8328 %x\n", snd_soc_read(codec, ES8328_DACCONTROL4));
	snd_soc_write(codec, ES8328_DACCONTROL5, (unsigned int)DigtVal); //0x1b
	dbg("ES8328 %x\n", snd_soc_read(codec, ES8328_DACCONTROL5));
		...
		struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	struct snd_soc_codec *codec = rtd->codec;
		...
		{
			//dbg("sound record!\n");
//			snd_soc_write(codec, ES8328_CHIPPOWER,  0x55);//0x0d
		}
		if (params_rate(params)== 44100)
		{
			//dbg("sound track!\n");
		//	snd_soc_write(codec, ES8328_CHIPPOWER,  0xaa);//0x0d
		}
...
static int es8328_dapm_event(struct snd_soc_codec *codec, int event)
{
	...
	case SNDRV_CTL_POWER_D0: /* full On */
		...
		snd_soc_write(codec, es8328_PWR1, pwr_reg | 0x00c0);
		break;
	case SNDRV_CTL_POWER_D1: /* partial On */
		snd_soc_write(codec, es8328_PWR1, pwr_reg | 0x01c0); 
		break;
	case SNDRV_CTL_POWER_D2: /* partial On */
		/* set vmid to 5k for quick power up */
		snd_soc_write(codec, es8328_PWR1, pwr_reg | 0x01c1);
		break;
	case SNDRV_CTL_POWER_D3hot: /* Off, with power */
		/* mute dac and set vmid to 500k, enable VREF */
		snd_soc_write(codec, es8328_PWR1, pwr_reg | 0x0141);
		break;
	case SNDRV_CTL_POWER_D3cold: /* Off, without power */
		snd_soc_write(codec, es8328_PWR1, 0x0001);
		break;
	}
...
#if 1 // add for MID suspend depop in earphone 
...
static int es8328_codec_init(struct snd_soc_codec *codec)
{
	...
	/* reset to default value.for i2c_shutdown will set to 0x06,but codec init never
	   reset to default value. while cause no audio issue when after charger mode boot MID. */
	es8328_set_bias_level(codec, SND_SOC_BIAS_STANDBY);
//	schedule_delayed_work(&codec->delayed_work, msecs_to_jiffies(1000));
#ifdef	CONFIG_HHTECH_MINIPMP
	// init first mutes
	//dbg("*********now, init every register!\n");
	snd_soc_write(codec, ES8328_MASTERMODE, 0x00);	// 0x08
	dbg("ES8328 ES8328_MASTERMODE,%x\n", snd_soc_read(codec, ES8328_MASTERMODE));
	//es8328_read(codec, ES8328_MASTERMODE);
	snd_soc_write(codec, ES8328_CHIPPOWER, 0xf3);  //0x02
	dbg("ES8328 ES8328_CHIPPOWER,%x\n", snd_soc_read(codec, ES8328_CHIPPOWER));
	//es8328_read(codec, ES8328_CONTROL1);
	snd_soc_write(codec,ES8328_CONTROL1 , 0x02);  //0x00
	dbg("ES8328 ES8328_CONTROL1%x\n", snd_soc_read(codec, ES8328_CONTROL1));
	//es8328_read(codec, ES8328_CHIPPOWER);

	snd_soc_write(codec, ES8328_CONTROL2, 0x040);	// 0x01
	 dbg("ES8328 ES8328_CONTROL2,%x\n", snd_soc_read(codec, ES8328_CONTROL2));
	 //snd_soc_write(codec, ES8328_DACPOWER, 0x30);
	 dbg("ES8328 ES8328_DACPOWER,%x\n", snd_soc_read(codec, ES8328_DACPOWER));
/*	
snd_soc_write(codec, ES8328_CONTROL2, 0x070);	// 0x01, low power
	snd_soc_write(codec, ES8328_CHIPLOPOW1, 0x0fb);	// 0x05, low power
	snd_soc_write(codec, ES8328_CHIPLOPOW2, 0x0ff);	// 0x06 low power
	snd_soc_write(codec, ES8328_ANAVOLMANAG, 0x07a);	// 0x07 low power
	*/
	snd_soc_write(codec, ES8328_ADCPOWER, 0x0000);	// 0x03
	dbg("ES8328 ES8328_ADCPOWER,%x\n", snd_soc_read(codec, ES8328_ADCPOWER));
	snd_soc_write(codec, ES8328_DACPOWER,  0x30);	// 0x04
	dbg("ES8328 ES8328_DACPOWER,%x\n", snd_soc_read(codec, ES8328_DACPOWER));
	snd_soc_write(codec, ES8328_ADCCONTROL1,  0x88);	// 0x09
	dbg("ES8328 ES8328_ADCCONTROL1,%x\n", snd_soc_read(codec, ES8328_ADCCONTROL1));
	snd_soc_write(codec, ES8328_ADCCONTROL2,  0xf0); //0x0a
	dbg("ES8328 ES8328_ADCCONTROL2,%x\n", snd_soc_read(codec, ES8328_ADCCONTROL2));
	snd_soc_write(codec, ES8328_ADCCONTROL3,  0x82);  //0x0b
	dbg("ES8328 ES8328_ADCCONTROL3%x\n", snd_soc_read(codec, ES8328_ADCCONTROL3));
	
#endif//CONFIG_HHTECH_MINIPMP
	/* for fpga */
	snd_soc_write(codec, ES8328_DACCONTROL21, 0x80); 
	dbg("ES8328 %x\n", snd_soc_read(codec, ES8328_DACCONTROL21));
	...
	dbg("ES8328 %x\n", snd_soc_read(codec, ES8328_ADCCONTROL4));
	//snd_soc_write(codec, ES8328_ADCCONTROL4,  0x10);  //0x0c
	snd_soc_write(codec, ES8328_ADCCONTROL5,  0x02);//0x0d
	dbg("ES8328 %x\n", snd_soc_read(codec, ES8328_ADCCONTROL5));
	//snd_soc_write(codec, ES8328_ADCCONTROL5,  0x00);//0x0d
	snd_soc_write(codec, ES8328_ADCCONTROL7,  0xf0);
	dbg("ES8328 %x\n", snd_soc_read(codec, ES8328_ADCCONTROL7));

	snd_soc_write(codec, ES8328_ADCCONTROL8, 0x00);//0x10
	dbg("ES8328 %x\n", snd_soc_read(codec, ES8328_ADCCONTROL8));
	snd_soc_write(codec, ES8328_ADCCONTROL9, 0x00);//0x11
	dbg("ES8328 %x\n", snd_soc_read(codec, ES8328_ADCCONTROL9));

	snd_soc_write(codec, ES8328_ADCCONTROL10,  0xea);//0x12
    dbg("ES8328 %x\n", snd_soc_read(codec, ES8328_ADCCONTROL10));

	
	snd_soc_write(codec, ES8328_ADCCONTROL11, 0xa0); //0x13
	dbg("ES8328 %x\n", snd_soc_read(codec, ES8328_ADCCONTROL11));
	snd_soc_write(codec, ES8328_ADCCONTROL12, 0x12); //0x14
	dbg("ES8328 %x\n", snd_soc_read(codec, ES8328_ADCCONTROL12));
	snd_soc_write(codec, ES8328_ADCCONTROL13, 0x06); //0x15
	dbg("ES8328 %x\n", snd_soc_read(codec, ES8328_ADCCONTROL13));
//	snd_soc_write(codec, ES8328_ADCCONTROL14, 0xc1); //0x16
	snd_soc_write(codec, ES8328_ADCCONTROL14, 0x93); //0x16
	dbg("ES8328 %x\n", snd_soc_read(codec, ES8328_ADCCONTROL14));
    if (item_equal("codec.data", "i2s", 0))
        snd_soc_write(codec, ES8328_DACCONTROL1, 0x98); //0x17
    else if (item_equal("codec.data", "pcm", 0))
        snd_soc_write(codec, ES8328_DACCONTROL1, 0x5E);
    dbg("ES8328 %x\n", snd_soc_read(codec, ES8328_DACCONTROL1));
	//snd_soc_write(codec, ES8328_DACCONTROL1, 0x20); //0x17
	snd_soc_write(codec, ES8328_DACCONTROL2, 0x02); //0x18
	dbg("ES8328 %x\n", snd_soc_read(codec, ES8328_DACCONTROL2));
	snd_soc_write(codec, ES8328_DACCONTROL4, 0x08); //0x1a
	dbg("ES8328 %x\n", snd_soc_read(codec, ES8328_DACCONTROL4));
    snd_soc_write(codec, ES8328_DACCONTROL5, 0x08); //0x1c
	...
	dbg("ES8328 %x\n", snd_soc_read(codec, ES8328_DACCONTROL16));
	snd_soc_write(codec, ES8328_DACCONTROL17, 0xb8); //0x27
	dbg("ES8328 %x\n", snd_soc_read(codec, ES8328_DACCONTROL17));
	snd_soc_write(codec, ES8328_DACCONTROL18, 0x38); //0x28
	dbg("ES8328 %x\n", snd_soc_read(codec, ES8328_DACCONTROL18));
	snd_soc_write(codec, ES8328_DACCONTROL19, 0x38); //0x29
	dbg("ES8328 %x\n", snd_soc_read(codec, ES8328_DACCONTROL19));
	snd_soc_write(codec, ES8328_DACCONTROL20, 0xb8); //0x2a
	dbg("ES8328 %x\n", snd_soc_read(codec, ES8328_DACCONTROL20));
    snd_soc_write(codec, ES8328_CHIPPOWER, 0x00);
    snd_soc_write(codec,ES8328_CONTROL1 , 0x06);
    snd_soc_write(codec, ES8328_DACPOWER, 0x30);
	snd_soc_write(codec, ES8328_DACCONTROL24, 0x1e); //0x2e
	dbg("ES8328 %x\n", snd_soc_read(codec, ES8328_DACCONTROL24));
	snd_soc_write(codec, ES8328_DACCONTROL25, 0x1e); //0x2f
	dbg("ES8328 %x\n", snd_soc_read(codec, ES8328_DACCONTROL25));
			    ...
			    const struct i2c_device_id *id)
{
    ...
    printk("buf is %s\n", buf);
#if 0
    es8328->regulator = regulator_get(&i2c->dev, buf);
    if (IS_ERR(es8328->regulator)) {
        printk("%s: get regulator fail\n", __func__);
        return -1;
    }
    ret = regulator_set_voltage(es8328->regulator, 2800000, 2800000);
    if(ret) {
        printk("%s: set regulator fail\n", __func__);
        return -1;
    }

    regulator_enable(es8328->regulator);
...
{
    struct es8328_priv *es8328 = i2c_get_clientdata(client);

    //regulator_put(es8328->regulator);
...
{                                                              
    ...
    imapx_gpio_spken(0);
#if 0                                    
    snd_soc_write(ES8328_codec, ES8328_CONTROL1, 0x32);     // 
    snd_soc_write(ES8328_codec, ES8328_DACCONTROL3, 0x06);     
    snd_soc_write(ES8328_codec, ES8328_DACPOWER, 0xc0); //0x04 
    snd_soc_write(ES8328_codec, ES8328_ADCPOWER, 0xFF); //0x03 
    snd_soc_write(ES8328_codec, ES8328_CHIPPOWER, 0xf3);//0x02 
    msleep(50);   
#endif
#if 1   // add for MID suspend depop in earphone 
snd_soc_write(codec, ES8328_DACCONTROL3, 0x06);
...
EXPORT_SYMBOL(es8328_i2c_driver);
#endif
