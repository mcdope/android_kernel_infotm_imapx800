...
typedef struct board_info {
	...
	volatile uint32_t *pdate_phyaddr;
        volatile uint32_t date_phyaddr;
	struct sk_buff *pskb;
...
static int gmac_reset(board_info_t *db)
{
	uint32_t ret = -1;
	uint32_t tmp, i = 0;
...
static void gmac_timeout(struct net_device *dev)
{
	...
	//phy_init(dev);
	//gmac_reset(db);
    //phy_init & mac_reset,but donot wait, fix kernel panic bug
    writel(BMCR_RESET, db->io_addr + rGMIIData);
    writel((0x1<<11) | (MII_BMCR<<6) | (0x1<<3) |(0x1<<1) | PHYBUSY, db->io_addr + rGMIIAddr );
...
static void gmac_tx_done(struct net_device *dev, board_info_t *db)
{
	...
	//dma_unmap_single(db->dev, db->pdate_phyaddr,
	//		 db->pskb->len, DMA_TO_DEVICE);
	//dev_kfree_skb(db->pskb);
	/* report tx available now */
	netif_wake_queue(dev);
#endif
}
...
static int gmac_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	db->date_phyaddr = dma_map_single(db->dev, skb->data, skb->len, DMA_TO_DEVICE);
	db->pdate_phyaddr = &(db->date_phyaddr);
	db->pskb = skb;

	gmac_cre_cur_tx_des(db, db->pdate_phyaddr, skb->len, db->eth_base_txdes_vir);
	...
	dma_unmap_single(db->dev, db->date_phyaddr,
...
static int gmac_drv_resume(struct platform_device *dev)
{
	...
	if (ndev)
	{
		if (netif_running(ndev))
		{
            gmac_hw_init();
            //phy_init(ndev);
            //phy_init,but donot wait, reduse 1min resume time
            writel(BMCR_RESET, db->io_addr + rGMIIData);
            writel((0x1<<11) | (MII_BMCR<<6) | (0x1<<3) |(0x1<<1) | PHYBUSY, db->io_addr + rGMIIAddr );
			if(gmac_reset(db) != 0){
				//TODO:	
...
static int __init gmac_init(void)
{
    ...
    if(item_exist("eth.usb.only") && item_integer("eth.usb.only",0))
        return -1;
