...
static uint8_t *mtd_page_buf = NULL;
...
static int mtd_open(struct inode *inode, struct file *file)
{
	...
	if(!mtd_page_buf) {
		mtd_page_buf = kmalloc(0x8000, GFP_KERNEL);
		if(!mtd_page_buf) {
			ret = -ENOMEM;
			goto out;
		}
	...
	case MEMPUTPAGE:
	{
		...
		if (copy_from_user(&buf, argp, sizeof(buf)))
		  ret = -EFAULT;
			...
			ops.datbuf = mtd_page_buf;
			ops.oobbuf = mtd_page_buf + mtd->writesize;
			ops.len    = mtd->writesize;
			ops.ooblen = 16;
			ops.ooboffs = 0;

			if(copy_from_user(mtd_page_buf,
							(uint8_t *)(uint32_t)buf.usr_ptr,
							mtd->writesize + 16)) {
				ret = -EFAULT;
				break;
			}
