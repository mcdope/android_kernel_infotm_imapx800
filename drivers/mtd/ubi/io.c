...
int ubi_io_read_page(const struct ubi_device *ubi, void *buf,
				   int pnum, int offset, int readpage)
{
	int err, retries = 0;
	size_t read;
	loff_t addr;
	...
	dbg_io("read page from PEB %d:%d", pnum, offset);
	...
	*((uint8_t *)buf) ^= 0xFF;
	...
	if(readpage) {
		ops.datbuf  = buf;
		ops.oobbuf  = buf + ubi->min_io_size;
	} else {
		ops.datbuf  = NULL;
		ops.oobbuf  = buf;
	}
...
retry:
	...
	if (err) {
		const char *errstr = (err == -EBADMSG) ? " (ECC error)" : "";
                ...
                ubi_err("addr = 0x%llx, ops.len = 0x%x, ops.ooblen = 0x%x, ops.datbuf = 0x%x ,ops.oobbuf = 0x%x", addr, ops_tmp.len, ops_tmp.ooblen, ops_tmp.datbuf, ops_tmp.oobbuf);
		...
		if (err == -EUCLEAN) {
			 ...
			 */
			dbg_msg("fixable bit-flip detected at PEB %d", pnum);
			ubi_assert(ubi->min_io_size == read);
			return UBI_IO_BITFLIPS;
		}

		if (retries++ < UBI_IO_RETRIES) {
			dbg_io("error %d%s while reading page from PEB %d:%d,"
			       " read only %zd bytes, retry",
			       err, errstr, pnum, offset, read);
			yield();
			goto retry;
		}

		ubi_err("error %d%s while reading page from PEB %d:%d, "
			"read %zd bytes", err, errstr, pnum, offset, read);
		ubi_dbg_dump_stack();
		 ...
		 * this, so we change it to -EIO.
		 */
		if (read != ubi->min_io_size && err == -EBADMSG) {
			ubi_assert(0);
			err = -EIO;
		}
	} else {
 ...
 * physical eraseblock.
 */
int ubi_io_write_page(struct ubi_device *ubi, const void *buf,
				    int pnum, int offset)
{
	int err, len;
	size_t written;
	...
	/* port from ubi_io_write */
	dbg_io("write page to PEB %d:%d", pnum, offset);
	...
	/* The area we are writing to has to contain all 0xFF bytes */
	err = ubi_dbg_check_all_ff(ubi, pnum, offset, ubi->min_io_size);
	if (err)
		return err;
	...
	if (ubi_dbg_is_write_failure()) {
		dbg_err("cannot write page to PEB %d:%d "
			"(emulated)", pnum, offset);
		ubi_dbg_dump_stack();
		return -EIO;
	}
	...
	written = ops.retlen;
	if (err) {
		ubi_err("error %d while writing page to PEB %d:%d, written "
			"%zd bytes", err, pnum, offset, written);
                ubi_err("addr = 0x%llx, ops.len = 0x%x, ops.ooblen = 0x%x, ops.datbuf = 0x%x ,ops.oobbuf = 0x%x", addr, ops_tmp.len, ops_tmp.ooblen, ops_tmp.datbuf, ops_tmp.oobbuf);
		ubi_dbg_dump_stack();
		ubi_dbg_dump_flash(ubi, pnum, offset, ubi->min_io_size);
	} else
		ubi_assert(written == ubi->min_io_size);

	if (!err) {
		err = ubi_dbg_check_write(ubi, buf, pnum, offset, ubi->min_io_size);
		if (err)
			return err;
