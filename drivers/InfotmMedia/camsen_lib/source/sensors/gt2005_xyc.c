...
-- v1.0.2	arsor@2012/07/16: reduce frame rate from 23.25fps to 18.6fps 
--			because brightness is too weak at 23.25fps.
--
------------------------------------------------------------------------------*/
...
#define INFOHEAD	"GT2005_XYC_I:"
#define WARNHEAD	"GT2005_XYC_W:"
#define ERRHEAD		"GT2005_XYC_E:"
#define TIPHEAD		"GT2005_XYC_T:"
...
typedef struct {
	...
	IM_INT32 bandMode;
	IM_INT32 sceneMode;
}gt2005_xyc_context_t;

static gt2005_xyc_context_t gGt2005;
...
int gt2005_xyc_switch_high_xuga(void){ 
	...
	{
		ret = cam_gt2005_write((IM_UINT8 *)(&gt2005_xuga_high_regs[i]));
		if(ret)
		{
...
int gt2005_xyc_switch_upmid_xuga(void){
	...
	{
		ret = cam_gt2005_write((IM_UINT8 *)(&gt2005_xuga_upmid_regs[i]));
		if(ret)
		{
			IM_ERRMSG((IM_STR("Failed to transfer data to i2c!")));
			return -1;
...
int gt2005_xyc_switch_mid_xuga(void){ 
	...
	{
		ret = cam_gt2005_write((IM_UINT8 *)(&gt2005_xuga_mid_regs[i]));
		if(ret)
		{
			IM_ERRMSG((IM_STR("Failed to transfer data to i2c!")));
			return -1;
...
int gt2005_xyc_switch_low_xuga(void){ 
	...
	{
		ret = cam_gt2005_write((IM_UINT8 *)(&gt2005_xuga_low_regs[i]));
		if(ret)
		{
			IM_ERRMSG((IM_STR("Failed to transfer data to i2c!")));
			return -1;
...
int gt2005_xyc_svga_to_xuga(void){
	...
	cam_gt2005_read((IM_UINT8 *)(&GT2005_0012), (uint8_t *)&reg0x0012, 1);   	
	cam_gt2005_read((IM_UINT8 *)(&GT2005_0013), (uint8_t *)&reg0x0013, 1);

	shutter = ((reg0x0012 << 8) |reg0x0013);
	
	cam_gt2005_read((IM_UINT8 *)(&GT2005_0014), (uint8_t *)&reg0x0014, 1);
	cam_gt2005_read((IM_UINT8 *)(&GT2005_0015), (uint8_t *)&reg0x0015, 1);

	AGain_shutter = ((reg0x0014 << 8) | reg0x0015);
	
	cam_gt2005_read((IM_UINT8 *)(&GT2005_0016), (uint8_t *)&reg0x0016, 1);
	cam_gt2005_read((IM_UINT8 *)(&GT2005_0017), (uint8_t *)&reg0x0017, 1);
...
int gt2005_xyc_to_preview_320_240(void){ 
	...
	{
		ret = cam_gt2005_write((IM_UINT8 *)(&gt2005_to_preview_320_240_regs[i]));
		if(ret)
		{
			IM_ERRMSG((IM_STR("Failed to transfer data to i2c!")));
			return -1;
...
int gt2005_xyc_to_preview_640_480(void){ 
	...
	{
		ret = cam_gt2005_write((IM_UINT8 *)(&gt2005_to_preview_640_480_regs[i]));
		if(ret)
		{
			IM_ERRMSG((IM_STR("Failed to transfer data to i2c!")));
			return -1;
...
int gt2005_xyc_to_preview_800_600(void){ 
	int i, ret;
	for(i = 0; i < (sizeof(gt2005_to_preview_800_600_regs) / 3); i++)
	{
		ret = cam_gt2005_write((IM_UINT8 *)(&gt2005_to_preview_800_600_regs[i]));
		if(ret)
		{
			IM_ERRMSG((IM_STR("Failed to transfer data to i2c!")));
			return -1;
...
IM_INT32 gt2005_xyc_set_sepia(void){ 
	IM_INT32 i, ret;
...
IM_INT32 gt2005_xyc_set_bluish(void){ 
	IM_INT32 i, ret;
...
IM_INT32 gt2005_xyc_set_greenish(void){
	IM_INT32 i, ret;
...
IM_INT32 gt2005_xyc_set_reddish(void){
	IM_INT32 i, ret;
...
IM_INT32 gt2005_xyc_set_yellowish(void){
	IM_INT32 i, ret;
...
IM_INT32 gt2005_xyc_set_bandw(void){ 
	IM_INT32 i, ret;
...
IM_INT32 gt2005_xyc_set_negative(void){
	IM_INT32 i, ret;
...
IM_INT32 gt2005_xyc_set_normal(void){ 
	IM_INT32 i, ret;
...
IM_INT32 gt2005_xyc_set_auto(void){ 
	IM_INT32 i, ret;
...
IM_INT32 gt2005_xyc_set_sunny(void){ 
	IM_INT32 i, ret;
...
IM_INT32 gt2005_xyc_set_cloudy(void){ 
	IM_INT32 i, ret;
...
IM_INT32 gt2005_xyc_set_office(void){ 
	IM_INT32 i, ret;
...
IM_INT32 gt2005_xyc_set_home(void){ 
	IM_INT32 i, ret;
...
IM_INT32 gt2005_xyc_set_effect(IM_INT32 value)
{
...
IM_INT32 gt2005_xyc_set_wb(IM_INT32 value)
{
	IM_INFOMSG((IM_STR("%s(value = %d)"), IM_STR(_IM_FUNC_), value));
	...
	switch(value)
	{
		...
		case CAM_WB_MODE_FLUORESCENT:
			gt2005_xyc_set_office();
			break;
		case CAM_WB_MODE_DAYLIGHT:
			gt2005_xyc_set_sunny();
			break;
		case CAM_WB_MODE_CLOUDY_DAYLIGHT:
			gt2005_xyc_set_cloudy();
			break;
...
IM_INT32 gt2005_xyc_set_night_mode(IM_BOOL enable)
{
	IM_INT32 i, ret;
...
IM_INT32 gt2005_xyc_set_scene_mode(IM_INT32 value)
{
	...
	return gt2005_xyc_set_night_mode(nightModeEn);
}

IM_INT32 gt2005_xyc_set_exposure(IM_INT32 value)
{
	IM_INFOMSG((IM_STR("%s(value = %d)"), IM_STR(_IM_FUNC_), value));
...
IM_INT32 gt2005_xyc_set_antibanding(IM_INT32 value)
{
	IM_INFOMSG((IM_STR("%s(value = %d)"), IM_STR(_IM_FUNC_), value));
...
//======================================================
IM_RET gt2005_xyc_pwdn(IM_UINT32 padNum)
{
...
IM_RET gt2005_xyc_get_pmu_info(camsenpwl_pmu_info_t *pmuInfo)
{
	IM_INFOMSG((IM_STR("%s()"), IM_STR(_IM_FUNC_)));
...
IM_RET gt2005_xyc_init(pwl_handle_t pwl, IM_BOOL checkOnly)
{
	IM_INT32 i, ret;
	IM_CHAR buf = 0;
	IM_INFOMSG((IM_STR("%s()++"), IM_STR(_IM_FUNC_)));
...
IM_RET gt2005_xyc_get_caps(camsen_caps_t *caps)
{
	IM_RET ret = IM_RET_OK;
	IM_INFOMSG((IM_STR("%s()"), IM_STR(_IM_FUNC_)));
...
IM_RET gt2005_xyc_get_out_mode(camsen_out_mode_t *outMode)
{
	IM_RET ret = IM_RET_OK;
	IM_INFOMSG((IM_STR("%s()"), IM_STR(_IM_FUNC_)));
...
IM_RET gt2005_xyc_set_out_mode(camsen_out_mode_t *outMode)
{
	IM_INT32 ret = 0;
	IM_UINT32 res, fps;
	IM_INFOMSG((IM_STR("%s()"), IM_STR(_IM_FUNC_)));
	...
	if(res == CAM_RES_UXGA)
	{
		...
		if(ret != 0)
		{
			IM_ERRMSG((IM_STR("gt2005_xyc_svga_to_uxga() failed!")));
			return IM_RET_FAILED;
		}
		ret = gt2005_xyc_switch_high_xuga();
		if(ret != 0)
		{
			IM_ERRMSG((IM_STR("gt2005_xyc_switch_high_uxga() failed!")));
			return IM_RET_FAILED;
		}
	}
	else if(res == CAM_RES_SVGA)
	{
		ret = gt2005_xyc_to_preview_800_600(); 
		if(ret != 0)
		{
			IM_ERRMSG((IM_STR("gt2005_xyc_to_preview_800_600() failed!")));
			return IM_RET_FAILED;
		}
	}
	else if(res == CAM_RES_VGA)
	{
		...
		if(ret != 0)
		{
			IM_ERRMSG((IM_STR("gt2005_xyc_to_preview_640_480() failed!")));
			return IM_RET_FAILED;
		}
	}
	else
...
IM_RET gt2005_xyc_set_property(IM_UINT32 property, void *p)
{
	IM_RET ret = IM_RET_OK;
	IM_INT32 value;
	IM_INFOMSG((IM_STR("%s()"), IM_STR(_IM_FUNC_)));
	...
	{
		gt2005_xyc_set_wb(value);
	}
	else if(property == CAM_KEY_RW_SPECIAL_EFFECT)
	{
		gt2005_xyc_set_effect(value);
	}
	else if(property == CAM_KEY_RW_SCENE_MODE)
	{
		gt2005_xyc_set_scene_mode(value);
	}
	else if(property == CAM_KEY_RW_EXPOSURE_COMPENSATION)
	{
		gt2005_xyc_set_exposure(value);
	}
	else if(property == CAM_KEY_RW_ANTIBANDING_MODE)
	{
		gt2005_xyc_set_antibanding(value);
	}

	return ret;
}

IM_RET gt2005_xyc_get_property(IM_UINT32 property, void *p)
{
	IM_RET ret = IM_RET_OK;
	IM_INT32 value = 0;
	IM_INFOMSG((IM_STR("%s()"), IM_STR(_IM_FUNC_)));
...
camsen_ops gt2005_xyc_ops = {
	.name                   = "gt2005_xyc",
	.i2c_dev_addr           = GT2005_I2C_ADDR,

	.sen_pwdn				= gt2005_xyc_pwdn,
	.sen_get_pmu_info		= gt2005_xyc_get_pmu_info,
	.sen_init				= gt2005_xyc_init,
	.sen_deinit				= gt2005_xyc_deinit,
	.sen_start				= gt2005_xyc_start,
	.sen_stop				= gt2005_xyc_stop,
	.sen_get_caps			= gt2005_xyc_get_caps,
	.sen_set_out_mode		= gt2005_xyc_set_out_mode,
	.sen_get_out_mode		= gt2005_xyc_get_out_mode,
	.sen_set_property		= gt2005_xyc_set_property,
	.sen_get_property		= gt2005_xyc_get_property,
};
