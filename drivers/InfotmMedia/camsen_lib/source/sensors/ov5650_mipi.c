...
#define INFOHEAD	"OV5650_MIPI_I:"
#define WARNHEAD	"OV5650_MIPI_W:"
#define ERRHEAD		"OV5650_MIPI_E:"
#define TIPHEAD		"OV5650_MIPI_T:"
...
typedef struct {
	...
	IM_INT32 sceneMode;
    IM_BOOL csiEn;
}ov5650_mipi_context_t;

static ov5650_mipi_context_t gOv5650;
...
//======================================================
IM_RET ov5650_mipi_pwdn(IM_UINT32 padNum)
{
...
IM_RET ov5650_mipi_get_pmu_info(camsenpwl_pmu_info_t *pmuInfo)
{
	IM_INFOMSG((IM_STR("%s()"), IM_STR(_IM_FUNC_)));
...
IM_INT32 ov5650_mipi_repeat_init()
{
...
IM_RET ov5650_mipi_init(pwl_handle_t pwl, IM_BOOL checkOnly)
{
	IM_INT32 ret;
    struct  ov5650_regval_list regs;
	IM_INFOMSG((IM_STR("%s()++"), IM_STR(_IM_FUNC_)));
...
IM_RET ov5650_mipi_deinit(void)
{
...
IM_RET ov5650_mipi_stop(void)
{
	IM_INFOMSG((IM_STR("%s()"), IM_STR(_IM_FUNC_)));
	return IM_RET_OK;
}

IM_RET ov5650_mipi_get_caps(camsen_caps_t *caps)
{
	IM_RET ret = IM_RET_OK;
	IM_INFOMSG((IM_STR("%s()"), IM_STR(_IM_FUNC_)));
	...
	caps->supportRes = CAM_RES_QVGA; 
	caps->maxRes = CAM_RES_QVGA; 
    caps->initRes = CAM_RES_QVGA;

	return ret;
}

IM_RET ov5650_mipi_get_out_mode(camsen_out_mode_t *outMode)
{
	IM_RET ret = IM_RET_OK;
	IM_INFOMSG((IM_STR("%s()"), IM_STR(_IM_FUNC_)));
...
IM_RET ov5650_mipi_set_out_mode(camsen_out_mode_t *outMode)
{
	IM_INT32 ret = 0;
	IM_UINT32 res, fps;
	IM_INFOMSG((IM_STR("%s()"), IM_STR(_IM_FUNC_)));
	...
	/*if(res == CAM_RES_VGA)
	{
		ret = ov5650_mipi_to_vga(); 
		if(ret != 0)
		{
			IM_ERRMSG((IM_STR("ov5650_mipi_to_vga() failed!")));
			return IM_RET_FAILED;
		}
	}
	else if(res == CAM_RES_720P)
	{
		ret = ov5650_mipi_to_720p(); 
		if(ret != 0)
		{
			IM_ERRMSG((IM_STR("ov5650_mipi_to_720p() failed!")));
			return IM_RET_FAILED;
		}
	}
	else if(res == CAM_RES_SXGA)
	{
		ret = ov5650_mipi_to_sxga(); 
		if(ret != 0)
		{
			IM_ERRMSG((IM_STR("ov5650_mipi_to_sxga() failed!")));
			return IM_RET_FAILED;
		}
	}
	else if(res == CAM_RES_1080P)
	{
		ret = ov5650_mipi_to_1080p(); 
		if(ret != 0)
		{
			IM_ERRMSG((IM_STR("ov5650_mipi_to_1080p() failed!")));
			return IM_RET_FAILED;
		}
	}
	else if(res == CAM_RES_500W)
	{
		ret = ov5650_mipi_to_qsxga(); 
		if(ret != 0)
		{
			IM_ERRMSG((IM_STR("ov5650_mipi_to_qsxga() failed!")));
			return IM_RET_FAILED;
		}
	}
	else
...
IM_RET ov5650_mipi_set_property(IM_UINT32 property, void *p)
{
	IM_RET ret = IM_RET_OK;
	IM_INT32 value;
	IM_INFOMSG((IM_STR("%s()"), IM_STR(_IM_FUNC_)));
	...
	{
		ov5650_mipi_set_wb(value);
	}
	else if(property == CAM_KEY_RW_SPECIAL_EFFECT)
	{
		ov5650_mipi_set_effect(value);
	}
	else if(property == CAM_KEY_RW_SCENE_MODE)
	{
		ov5650_mipi_set_scene_mode(value);
	}
	else if(property == CAM_KEY_RW_FLASH_MODE)
	{
		ov5650_mipi_set_flash_mode(value);
	}
	else if(property == CAM_KEY_RW_LIGHT_TURN_ON)
	{
		ov5650_mipi_set_light_on(value);
	}
	else if(property == CAM_KEY_RW_EXPOSURE_COMPENSATION)
	{
		ov5650_mipi_set_exposure(value);
	}
	else if(property == CAM_KEY_RW_ANTIBANDING_MODE)
	{
		ov5650_mipi_set_band_filter(value);
	}*/

	return ret;
}

IM_RET ov5650_mipi_get_property(IM_UINT32 property, void *p)
{
	IM_RET ret = IM_RET_OK;
	IM_INT32 value = 0;
	IM_INFOMSG((IM_STR("%s()"), IM_STR(_IM_FUNC_)));
...
camsen_ops ov5650_mipi_ops = {
	.name                   = "ov5650_mipi",
	.i2c_dev_addr           = OV5650_I2C_ADDR,

	.sen_pwdn				= ov5650_mipi_pwdn,
	.sen_get_pmu_info		= ov5650_mipi_get_pmu_info,
	.sen_init				= ov5650_mipi_init,
	.sen_deinit				= ov5650_mipi_deinit,
	.sen_start				= ov5650_mipi_start,
	.sen_stop				= ov5650_mipi_stop,
	.sen_get_caps			= ov5650_mipi_get_caps,
	.sen_set_out_mode		= ov5650_mipi_set_out_mode,
	.sen_get_out_mode		= ov5650_mipi_get_out_mode,
	.sen_set_property		= ov5650_mipi_set_property,
	.sen_get_property		= ov5650_mipi_get_property,
};
