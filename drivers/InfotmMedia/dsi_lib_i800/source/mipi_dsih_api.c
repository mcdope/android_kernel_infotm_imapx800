...
static const IM_UINT32 mipi_dsih_supported_versions[] = {0x3130312A};
static const IM_UINT32 mipi_dsih_no_of_versions = sizeof(mipi_dsih_supported_versions) / sizeof(IM_UINT32);
...
dsih_error_t mipi_dsih_open(dsih_ctrl_t * instance)
{
	...
	//mipi_dsih_hal_power(instance, 1);//sam :
	...
	//mipi_dsih_unregister_all_events(instance);  //sam : mask all the interrupts. for rtl case.
 ...
 */
void mipi_dsih_allow_return_to_lp(dsih_ctrl_t * instance, IM_INT32 hfp, IM_INT32 hbp, IM_INT32 vactive, IM_INT32 vfp, IM_INT32 vbp, IM_INT32 vsync)
{
	if (instance != 0)
	{
		if (instance->status == INITIALIZED)
 ...
 */
void mipi_dsih_dcs_cmd_lp_transmission(dsih_ctrl_t * instance, IM_INT32 long_write, IM_INT32 short_write, IM_INT32 short_read)
{
	if (instance != 0)
	{
		if (instance->status == INITIALIZED)
 ...
 */
void mipi_dsih_gen_cmd_lp_transmission(dsih_ctrl_t * instance, IM_INT32 long_write, IM_INT32 short_write, IM_INT32 short_read)
{
	if (instance != 0)
	{
		if (instance->status == INITIALIZED)
 ...
 * @return error code
 */
dsih_error_t mipi_dsih_enable_rx(dsih_ctrl_t * instance, IM_INT32 enable)
{
 ...
 * @return error code
 */
dsih_error_t mipi_dsih_peripheral_ack(dsih_ctrl_t * instance, IM_INT32 enable)
{
	if (instance != 0)
	{
		if (instance->status == INITIALIZED)
 ...
 * @return error code
 */
dsih_error_t mipi_dsih_tear_effect_ack(dsih_ctrl_t * instance, IM_INT32 enable)
{
	if (instance != 0)
	{
		if (instance->status == INITIALIZED)
 ...
 * @return error code
 */
dsih_error_t mipi_dsih_eotp_rx(dsih_ctrl_t * instance, IM_INT32 enable)
{
	if (instance != 0)
	{
		if (instance->status == INITIALIZED)
 ...
 * @return error code
 */
dsih_error_t mipi_dsih_ecc_rx(dsih_ctrl_t * instance, IM_INT32 enable)
{
	if (instance != 0)
	{
		if (instance->status == INITIALIZED)
 ...
 * @return error code
 */
dsih_error_t mipi_dsih_eotp_tx(dsih_ctrl_t * instance, IM_INT32 enable)
{
	if (instance != 0)
	{
		if (instance->status == INITIALIZED)
...
{
	dsih_error_t err_code = DSI_OK;
	IM_UINT16 bytes_per_pixel_x100 = 0;
	IM_UINT16 video_size = 0;
	IM_UINT32 total_line_bytes = 0;
	IM_UINT32 no_of_chunks = 0;
	IM_UINT32 ratio_clock_xPF = 0; /* holds dpi clock/byte clock times precision factor */
	IM_UINT16 null_packet_size = 0;
	IM_UINT8 video_size_step = 1;
	//IM_UINT32 hs_timeout  = 0;
	//IM_INT32 counter = 0;
	...
	mipi_dsih_hal_dpi_hline(instance, (IM_UINT16)((video_params->h_total_pixels * ratio_clock_xPF) / PRECISION_FACTOR));
...
#else
		mipi_dsih_hal_lp_rx_timeout(instance, (IM_UINT16)0x1FFF); // For rtl test need
		mipi_dsih_hal_hs_tx_timeout(instance, (IM_UINT16)0x1FFF); // For rtl test need
#endif
#else
	...
	for (counter = 0xff; (counter < hs_timeout) && (counter > 2); counter--)
	{
		if ((hs_timeout % counter) == 0)
		{
			...
			mipi_dsih_hal_lp_rx_timeout(instance, (IM_UINT16)(hs_timeout / counter));
			mipi_dsih_hal_hs_tx_timeout(instance, (IM_UINT16)(hs_timeout / counter));
			break;
		}
	}
	...
	{ /* NON BURST */
		...
		{/* MULTI */
			...
			if (instance->log_info != 0)
			{
				instance->log_info("h line time %ld", (IM_UINT16)((video_params->h_total_pixels * ratio_clock_xPF) / PRECISION_FACTOR));
...
/**
 * Configure DBI video interface
 * @param instance pointer to structure holding the DSI Host core information
 * @param video_params pointer to video stream-to-send information
 * @return error code
 */
dsih_error_t mipi_dsih_dbi_video(dsih_ctrl_t * instance, dsih_dbi_video_t * video_params)
{
	dsih_error_t err_code = DSI_OK;
	...
	mipi_dsih_hal_timeout_clock_division(instance, 1);
	mipi_dsih_hal_tx_escape_timeout(instance, 0x0c);
	// timeout transmission or reception. may be used in the event test.
	mipi_dsih_hal_lp_rx_timeout(instance, (IM_UINT16)0x1FFF);
	mipi_dsih_hal_hs_tx_timeout(instance, (IM_UINT16)0x1FFF);
	mipi_dsih_dphy_no_of_lanes(&(instance->phy_instance), video_params->no_of_lanes);

	mipi_dsih_hal_dbi_out_color_coding(instance, video_params->out_dbi_conf, 0);
	mipi_dsih_hal_dbi_in_color_coding(instance, video_params->in_dbi_conf, 0);
	mipi_dsih_hal_dbi_lut_size(instance , video_params->lut_size_conf);
 	mipi_dsih_hal_dbi_partitioning_en(instance, video_params->partitioning_en);	
	mipi_dsih_hal_dbi_dcs_vc(instance , video_params->dbi_vid);
	mipi_dsih_hal_dbi_max_cmd_size(instance , video_params->allowed_cmd_size);
	mipi_dsih_hal_dbi_cmd_size(instance, video_params->wr_cmd_size);
	return err_code;
}
 ...
 * @return error code
 */
dsih_error_t mipi_dsih_dcs_wr_cmd(dsih_ctrl_t * instance, IM_UINT8 vc, IM_UINT8* params, IM_UINT16 param_length)
{
	IM_UINT8 packet_type = 0;
	if (instance == 0)
	{
		return ERR_DSI_INVALID_INSTANCE;
	}
 ...
 */
void mipi_dsih_cmd_mode(dsih_ctrl_t * instance, IM_INT32 en)
{
	if (instance != 0)
	{
		if (instance->status == INITIALIZED)
...
/**
 * sam : Enable video mode
 * @param instance pointer to structure holding the DSI Host core information
 * @param en enable/disable
 */
void mipi_dsih_video_mode(dsih_ctrl_t * instance, IM_INT32 en)
{
	if (instance != 0)
	{
		if (instance->status == INITIALIZED)
 ...
 */
dsih_error_t mipi_dsih_gen_wr_cmd(dsih_ctrl_t * instance, IM_UINT8 vc, IM_UINT8 command_type, IM_UINT8* params, IM_UINT16 param_length)
{
	dsih_error_t err_code = DSI_OK;
	IM_INT32 timeout = 0;
	IM_INT32 i = 0;
	IM_INT32 j = 0;
	IM_INT32 compliment_counter = 0;
	IM_UINT8* payload = 0;
	IM_UINT32 temp = 0;
	IM_UINT16 word_count = 0;
	if (instance == 0)
	{
		return ERR_DSI_INVALID_INSTANCE;
	}
	...
	if (param_length > 2)
	{
		...
		payload = params + (2 * sizeof(IM_UINT8));
 ...
 */
IM_UINT16 mipi_dsih_dcs_rd_cmd(dsih_ctrl_t * instance, IM_UINT8 vc, IM_UINT8 command, IM_UINT8 bytes_to_read, IM_UINT8* read_buffer)
{
	if (instance == 0)
	{
		return 0;
 ...
 * @note this function will enable BTA
 */
IM_UINT16 mipi_dsih_gen_rd_cmd(dsih_ctrl_t * instance, IM_UINT8 vc, IM_UINT8 command, IM_UINT8 msb_byte, IM_UINT8 lsb_byte, IM_UINT8 bytes_to_read, IM_UINT8* read_buffer)
{
	dsih_error_t err_code = DSI_OK;
	IM_INT32 timeout = 0;
	IM_INT32 counter = 0;
		...
		if (!mipi_dsih_hal_gen_rd_cmd_busy(instance))
		{
			for (counter = 0; counter < bytes_to_read; counter += 4)
			{
				...
				{	/* put 32 bit temp in 4 bytes of buffer passed by user*/
					read_buffer[counter + i] = (IM_UINT8)(temp[0] >> (i * 8));
				}
			}
		}
	}
 ...
 */
IM_UINT32 mipi_dsih_dump_register_configuration(dsih_ctrl_t * instance, IM_INT32 all, register_config_t *config, IM_UINT16 config_length)
{
	IM_UINT32 curr;
	IM_UINT16 count;
	curr = 0;
	...
	if (all)
	{ /* dump all registers*/
		for (curr = R_DSI_HOST_VERSION; curr <= R_DSI_HOST_ERROR_MSK1; count++, curr += (R_DSI_HOST_PWR_UP - R_DSI_HOST_VERSION))
		{
			if ((config_length == 0) || (config == 0) || count >= config_length)
			{ /* no place to write - write to STD IO*/
				if (instance->log_info != 0)
				{
					instance->log_info("DSI 0x%lX:0x%lX", curr, mipi_dsih_read_word(instance, curr));
				}
			}
			else
			{
				config[count].addr = curr;
				config[count].data = mipi_dsih_read_word(instance, curr);
			}
		}
	}
	else
 ...
 */
IM_UINT32 mipi_dsih_write_register_configuration(dsih_ctrl_t * instance, register_config_t *config, IM_UINT16 config_length)
{
	IM_UINT16  count = 0;
	if (instance == 0)
	{
...
void mipi_dsih_event_handler(void * param)
{
	...
	IM_UINT8 i = 0;
	IM_UINT32 status_0 = mipi_dsih_hal_error_status_0(instance, 0xffffffff);
	IM_UINT32 status_1 = mipi_dsih_hal_error_status_1(instance, 0xffffffff);
	if (instance == 0)
	{
		return;
	}
 ...
 */
void mipi_dsih_shutdown_controller(dsih_ctrl_t * instance, IM_INT32 shutdown)
{
 ...
 * @param shutdown (1) power up (0)
 */
void mipi_dsih_shutdown_phy(dsih_ctrl_t * instance, IM_INT32 shutdown)
{
