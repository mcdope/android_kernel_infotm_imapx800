...
dsih_error_t mipi_dsih_dphy_configure(dphy_t * phy, IM_UINT8 no_of_lanes, IM_UINT32 output_freq)
{
	...
	IM_UINT8 data[4]; /* maximum data for now are 4 bytes per test mode*/
	IM_UINT32 divider = 0;
	IM_UINT8 i = 0;
	IM_UINT8 no_of_bytes = 0;
	...
	{
		data[i] = ((IM_UINT8)(divider >> (8 * i)) | (i << 7) );
		/* 7 is dependent on no_of_bytes
		(7 = sizeof(IM_UINT8) - min_needed_bits to write no_of_bytes)*/
	}
	...
	/*TODO - check */
	//sam : Or may add check here, before mipi_dsih_dphy_if_control();
...
#ifdef DWC_MIPI_DPHY_BIDIR_TESTCHIP
	mipi_dsih_dphy_reset(phy, 1);
	mipi_dsih_dphy_clock_en(phy, 1);
	mipi_dsih_dphy_shutdown(phy, 1);
	mipi_dsih_dphy_no_of_lanes(phy, no_of_lanes);
...
void mipi_dsih_dphy_write_part(dphy_t * instance, IM_UINT32 reg_address, IM_UINT32 data, IM_UINT8 shift, IM_UINT8 width)
{
	...
	IM_UINT32 temp = mipi_dsih_dphy_read_word(instance, reg_address);
	temp &= ~(mask << shift);
