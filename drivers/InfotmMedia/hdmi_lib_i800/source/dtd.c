...
extern IM_INT32 edid_video_cb(dtd_t *dtd);
...
#if 0
	printk("sam : dtd->mHActive = %d ,mVActive = %d , mPixelClock = %d - \n",dtd->mHActive,dtd->mVActive ,dtd->mPixelClock);
	printk("    HBlanking=%d, mHSyncOffset=%d, mHSyncPulseWidth=%d, mHImageSize=%d, mHBorder=%d, - \n",dtd->mHBlanking,dtd->mHSyncOffset,dtd->mHSyncPulseWidth,dtd->mHImageSize,dtd->mHBorder);
	printk("    VBlanking=%d, mVSyncOffset=%d, mVSyncPulseWidth=%d -mVImageSize=%d, mVBorder=%d  \n",dtd->mVBlanking,dtd->mVSyncOffset,dtd->mVSyncPulseWidth,dtd->mVImageSize,dtd->mVBorder);
	printk("    mInterlaced=%d , mVSyncPolarity=%d, mHSyncPolarity=%d , -\n ",dtd->mInterlaced,dtd->mVSyncPolarity,dtd->mHSyncPolarity);
	printk("    #################################################\n");
#endif
