...
#define shaCircularShift(bits,word) ((((word) << (bits)) & 0xFFFFFFFF) | ((word) >> (32-(bits))))
	...
	IM_UINT32 W[80]; /* word sequence */
	IM_UINT32 A, B, C, D, E; /* word buffers */
	IM_UINT32 temp = 0;
	IM_INT32 t = 0;
...
IM_INT32 hdcpVerify_ComputeMUL(IM_UINT8 * p, const IM_UINT8 * a, const IM_UINT8 * b, const IM_UINT8 * m,
		size_t n)
{
	IM_UINT8 aux[2*KSIZE + 1];
	IM_UINT8 ext[KSIZE + 1];
	IM_UINT8 tmp[2* (KSIZE +1)];
	size_t i = 0;
	IM_INT32 j = 0;
	if (n > KSIZE)
