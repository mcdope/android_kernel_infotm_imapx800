...
int hy252_xx_reset(void __iomem *reg)
{
...
int hy252_xx_set_mode(int value, void __iomem *reg)
{
	int ret;
...
static struct res_to_fps_map hy252_xx_rf_maps[3] = {
	{
...
static uint32_t hy252_xx_get_fps(uint32_t res)
{
	int i = 0;
	uint32_t fps = 0;
	...
	for ( ; i < (sizeof(hy252_xx_rf_maps) / sizeof(hy252_xx_rf_maps[0])); i++) {
		if (hy252_xx_rf_maps[i].res == res) {
			fps = hy252_xx_rf_maps[i].fps;
			break;
		}
	}
