/* linux/arch/arm/plat-samsung/imap-pl330.c
 *
...
#include <linux/imapx800-pl330.h>
#include <mach/power-gate.h>
/**
 * struct imap_pl330_dmac - Logical representation of a PL330 DMAC.
...
/**
 * struct imap_pl330_xfer - A request submitted by imap DMA clients.
 ...
 */
struct imap_pl330_xfer {
	void			*token;
	struct list_head	node;
	struct pl330_xfer	px;
	struct imap_pl330_chan	*chan;
};

/**
 * struct imap_pl330_chan - Logical channel to communicate with
 ...
 * @sdaddr: Address provided via imapx800_dma_devconfig.
...
struct imap_pl330_chan {
	...
	imapx800_dma_cbfn_t		callback_fn;
	struct pl330_reqcfg		rqcfg;
	struct imap_pl330_xfer		*xfer_head;
	struct imap_pl330_dmac		*dmac;
...
static struct imap_pl330_chan *id_to_chan(const enum dma_ch id)
{
	struct imap_pl330_chan *ch;
...
{
	struct imap_pl330_chan *ch = id_to_chan(id);
...
static bool chan_free(struct imap_pl330_chan *ch)
{
	if (!ch)
		return false;
 ...
 * Index + 1, otherwise.
 */
static unsigned iface_of_dmac(struct imap_pl330_dmac *dmac, enum dma_ch ch_id)
{
	enum dma_ch *id = dmac->peri;
	int i;

	/* Discount invalid markers */
	if (ch_id == IMAPX800_MAX)
		return 0;
...
static inline bool dmac_busy(struct imap_pl330_dmac *dmac)
{
 ...
 */
static unsigned ch_onlyby_dmac(struct imap_pl330_dmac *dmac)
{
	enum dma_ch *id = dmac->peri;
	struct imap_pl330_dmac *d;
	struct imap_pl330_chan *ch;
	...
	for (i = 0; i < PL330_MAX_PERI; i++) {
		...
		if (p == IMAPX800_MAX || !chan_free(ch))
			continue;
...
static unsigned suitablility(struct imap_pl330_dmac *dmac,
		struct imap_pl330_chan *ch)
{
	struct pl330_info *pi = dmac->pi;
	enum dma_ch *id = dmac->peri;
 ...
 */
static struct imap_pl330_dmac *map_chan_to_dmac(struct imap_pl330_chan *ch)
{
	struct imap_pl330_dmac *d, *dmac = NULL;
...
static struct imap_pl330_chan *chan_acquire(const enum dma_ch id)
{
	struct imap_pl330_chan *ch = id_to_chan(id);
	struct imap_pl330_dmac *dmac;
...
static inline void del_from_queue(struct imap_pl330_xfer *xfer)
{
	struct imap_pl330_xfer *t;
	struct imap_pl330_chan *ch;
	int found;
 ...
 */
static struct imap_pl330_xfer *get_from_queue(struct imap_pl330_chan *ch,
		int pluck)
{
	struct imap_pl330_xfer *xfer = ch->xfer_head;
	...
	if (pluck || !(ch->options & IMAPX800_DMAF_CIRCULAR))
		del_from_queue(xfer);

	return xfer;
}

static inline void add_to_queue(struct imap_pl330_chan *ch,
		struct imap_pl330_xfer *xfer, int front)
{
	...
	if (ch->options & IMAPX800_DMAF_CIRCULAR &&
...
static inline void _finish_off(struct imap_pl330_xfer *xfer,
		enum imapx800_dma_buffresult res, int ffree)
{
	struct imap_pl330_chan *ch;
	...
	if (ffree || !(ch->options & IMAPX800_DMAF_CIRCULAR))
		kmem_cache_free(ch->dmac->kmcache, xfer);
}

static inline int imap_pl330_submit(struct imap_pl330_chan *ch,
		struct pl330_req *r)
{
	struct imap_pl330_xfer *xfer;
	int ret = 0;
	...
	if (xfer) {
		...
		if (ret == -EAGAIN) {
			...
			dev_err(ch->dmac->pi->dev, "%s:%d!\n",
				__func__, __LINE__);
			_finish_off(xfer, IMAPX800_RES_ERR, 0);
		}
	}
...
static void imap_pl330_rq(struct imap_pl330_chan *ch,
	struct pl330_req *r, enum pl330_op_err err)
{
	unsigned long flags;
	struct imap_pl330_xfer *xfer;
	struct pl330_xfer *xl = r->x;
	enum imapx800_dma_buffresult res;

	spin_lock_irqsave(&res_lock, flags);

	r->x = NULL;

	imap_pl330_submit(ch, r);

	spin_unlock_irqrestore(&res_lock, flags);

	/* Map result to imap DMA API */
	if (err == PL330_ERR_NONE)
		res = IMAPX800_RES_OK;
	else if (err == PL330_ERR_ABORT)
		res = IMAPX800_RES_ABORT;
	else
		res = IMAPX800_RES_ERR;

	/* If last request had some xfer */
	if (xl) {
		xfer = container_of(xl, struct imap_pl330_xfer, px);
...
static void imap_pl330_rq0(void *token, enum pl330_op_err err)
{
	struct pl330_req *r = token;
	struct imap_pl330_chan *ch = container_of(r,
					struct imap_pl330_chan, req[0]);
	imap_pl330_rq(ch, r, err);
}

static void imap_pl330_rq1(void *token, enum pl330_op_err err)
{
	struct pl330_req *r = token;
	struct imap_pl330_chan *ch = container_of(r,
					struct imap_pl330_chan, req[1]);
	imap_pl330_rq(ch, r, err);
}

/* Release an acquired channel */
static void chan_release(struct imap_pl330_chan *ch)
{
	struct imap_pl330_dmac *dmac;
...
int imapx800_dma_ctrl(enum dma_ch id, enum imapx800_chan_op op)
{
	struct imap_pl330_xfer *xfer;
	...
	switch (op) {
	case IMAPX800_DMAOP_START:
		/* Make sure both reqs are enqueued */
		idx = (ch->lrq == &ch->req[0]) ? 1 : 0;
		imap_pl330_submit(ch, &ch->req[idx]);
		imap_pl330_submit(ch, &ch->req[1 - idx]);
		pl330op = PL330_OP_START;
		break;

	case IMAPX800_DMAOP_STOP:
		pl330op = PL330_OP_ABORT;
		break;

	case IMAPX800_DMAOP_FLUSH:
		pl330op = PL330_OP_FLUSH;
		break;

	case IMAPX800_DMAOP_PAUSE:
	case IMAPX800_DMAOP_RESUME:
	case IMAPX800_DMAOP_TIMEOUT:
	case IMAPX800_DMAOP_STARTED:
		spin_unlock_irqrestore(&res_lock, flags);
		return 0;
	...
	if (ch->req[idx].x) {
		...
		spin_unlock_irqrestore(&res_lock, flags);
		_finish_off(xfer, IMAPX800_RES_ABORT,
	...
	if (pl330op == PL330_OP_FLUSH) {
		...
		if (ch->req[1 - idx].x) {
			...
			spin_unlock_irqrestore(&res_lock, flags);
			_finish_off(xfer, IMAPX800_RES_ABORT, 1);
			spin_lock_irqsave(&res_lock, flags);
		}
...
EXPORT_SYMBOL(imapx800_dma_ctrl);

int imapx800_dma_enqueue(enum dma_ch id, void *token,
			dma_addr_t addr, int size)
{
	...
	/* For imap DMA API, direction is always fixed for all xfers */
	...
	if (ch->options & IMAPX800_DMAF_AUTOSTART)
		imapx800_dma_ctrl(id, IMAPX800_DMAOP_START);
...
EXPORT_SYMBOL(imapx800_dma_enqueue);

int imapx800_dma_request(enum dma_ch id,
			struct imapx800_dma_client *client,
			void *dev)
{
	struct imap_pl330_dmac *dmac;
	...
	ch->req[0].xfer_cb = imap_pl330_rq0;
	ch->req[1].token = &ch->req[1];
	ch->req[1].xfer_cb = imap_pl330_rq1;
...
EXPORT_SYMBOL(imapx800_dma_request);

int imapx800_dma_free(enum dma_ch id, struct imapx800_dma_client *client)
{
	struct imap_pl330_chan *ch;
	struct imap_pl330_xfer *xfer;
	unsigned long flags;
...
EXPORT_SYMBOL(imapx800_dma_free);

int imapx800_dma_config(enum dma_ch id, int xferunit, int brst_len)
{
	struct imap_pl330_chan *ch;
	struct pl330_info *pi;
	unsigned long flags;
	...
	while (xferunit != (1 << i)) {
		i++;
        /* when xferunit is odd, it will in dead loop */
        if ((1 << i) > xferunit)
            break;
    }
...
EXPORT_SYMBOL(imapx800_dma_config);

/* Options that are supported by this driver */
#define IMAPX_PL330_FLAGS (IMAPX800_DMAF_CIRCULAR | IMAPX800_DMAF_AUTOSTART)

int imapx800_dma_setflags(enum dma_ch id, unsigned int options)
{
	struct imap_pl330_chan *ch;
	unsigned long flags;
	int ret = 0;
	...
	if (!ch || chan_free(ch) || options & ~(IMAPX_PL330_FLAGS))
		ret = -EINVAL;
	else
		ch->options = options;
...
EXPORT_SYMBOL(imapx800_dma_setflags);

int imapx800_dma_set_buffdone_fn(enum dma_ch id, imapx800_dma_cbfn_t rtn)
{
	struct imap_pl330_chan *ch;
	unsigned long flags;
	int ret = 0;
...
EXPORT_SYMBOL(imapx800_dma_set_buffdone_fn);

int imapx800_dma_devconfig(enum dma_ch id, enum imapx800_dmasrc source,
			  unsigned long address)
{
	...
	switch (source) {
	case IMAPX800_DMASRC_HW: /* P->M */
		ch->req[0].rqtype = DEVTOMEM;
		ch->req[1].rqtype = DEVTOMEM;
		ch->rqcfg.src_inc = 0;
		ch->rqcfg.dst_inc = 1;
		break;
	case IMAPX800_DMASRC_MEM: /* M->P */
...
EXPORT_SYMBOL(imapx800_dma_devconfig);

int imapx800_dma_getposition(enum dma_ch id, dma_addr_t *src, dma_addr_t *dst)
{
	struct imap_pl330_chan *ch = id_to_chan(id);
...
EXPORT_SYMBOL(imapx800_dma_getposition);
...
{
	struct imap_pl330_dmac *imap_pl330_dmac;
	struct imapx800_pl330_platdata *pl330pd;
    ...
    for (j = 0;j < 13;j++) {
        irq[j] = platform_get_irq(pdev, j);
        if (irq[j] < 0) {
            ret = irq[j];
            goto probe_err3;
        }
        ret = request_irq(irq[j], pl330_irq_handler, 0,
	...
	/* Allocate a new DMAC */
	imap_pl330_dmac = kmalloc(sizeof(*imap_pl330_dmac), GFP_KERNEL);
	if (!imap_pl330_dmac) {
		ret = -ENOMEM;
		goto probe_err6;
	}
	...
	/* Hook the info */
	imap_pl330_dmac->pi = pl330_info;

	/* No busy channels */
	imap_pl330_dmac->busy_chan = 0;

	imap_pl330_dmac->kmcache = kmem_cache_create(dev_name(&pdev->dev),
				sizeof(struct imap_pl330_xfer), 0, 0, NULL);

	if (!imap_pl330_dmac->kmcache) {
		ret = -ENOMEM;
		goto probe_err7;
	}

	/* Get the list of peripherals */
	imap_pl330_dmac->peri = pl330pd->peri;

	/* Attach to the list of DMACs */
	list_add_tail(&imap_pl330_dmac->node, &dmac_list);

	/* Create a channel for each peripheral in the DMAC
	 * that is, if it doesn't already exist
	 */
	for (i = 0; i < PL330_MAX_PERI; i++)
		if (imap_pl330_dmac->peri[i] != IMAPX800_MAX)
			chan_add(imap_pl330_dmac->peri[i]);
...
static int pl330_remove(struct platform_device *pdev)
{
	...
	list_for_each_entry(ch, &chan_list, node) {
		...
		if (del) {
			spin_unlock_irqrestore(&res_lock, flags);
			imapx800_dma_free(ch->id, ch->client);
			spin_lock_irqsave(&res_lock, flags);
			list_del(&ch->node);
			kfree(ch);
		}
