		 ...
		 struct dma_tx_state *txstate)
{
	...
	if (ret !=  DMA_SUCCESS)
		dma_set_tx_state(txstate, last_done, last_used, pl330_get_len(chan));		
	else		
...
static int pl330_dma_getposition(struct dma_chan *chan, dma_addr_t *src, dma_addr_t *dst)
{
	struct dma_pl330_chan *pch = to_pchan(chan);
	struct pl330_chanstatus status;
		...
		size_t period_len, enum dma_data_direction direction)
{
	...
	fill_px(&desc->px, dst, src, period_len, period_len);
...
static int pl330_max(uint8_t a, uint8_t b)
{
    if (a > b)
        return a;
    else
...
pl330_probe(struct amba_device *adev, const struct amba_id *id)
{
	...
	int i, ret, j = 0;
	int num_chan;
    bool set_flag = true;      //set dma irq to cpu1

    cpumask_var_t mask;
    mask->bits[0] = 0x2;
#if 0 
    if (item_exist("gps.model"))
        set_flag = false;
#endif
	set_secure_para(1);
    ...
    while (adev->irq[j] != -1) {
        ret = request_irq(adev->irq[j], pl330_irq_handler, 0,
                dev_name(&adev->dev), pi);
        if (set_flag) {
            ret = irq_set_affinity(adev->irq[j], mask);             
            if(ret){                                                
                dev_err(&adev->dev, "dma %d irq set affinity cpu1 error \n", j);
                goto probe_err2;                                    
            }
        }
        j++;
	...
	num_chan = pl330_max(pdat ? pdat->nr_valid_peri : 0, (uint8_t)pi->pcfg.num_chan);
	...
	pd->device_dma_getposition = pl330_dma_getposition;
...
#ifdef CONFIG_PM
static int pl330_suspend(struct amba_device *pdev, pm_message_t state)
{
    return 0;
}

static int pl330_resume(struct amba_device *pdev)
{
    cpumask_var_t mask;
    int j = 0, ret;
    mask->bits[0] = 0x2;
    ...
    set_secure_para(1);
    while(pdev->irq[j] != -1) {
        ret = irq_set_affinity(pdev->irq[j], mask);
        if(ret){
            dev_err(&pdev->dev, "dma %d irq set affinity cpu1 error \n", j);
            return -1;
        }
        j++;
    }
...
#else
#define pl330_suspend NULL
#define pl330_resume NULL
#endif
