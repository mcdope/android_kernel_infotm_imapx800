...
//#define DEBUG
#ifdef DEBUG
#define ITEMS_DBG(x...) printk(x)
#else
#define ITEMS_DBG(x...) do{} while(0)
#endif
			...
			unsigned int cmd, unsigned long arg)                       
{
	...
	switch(cmd)
	{
		case ITEMS_EXIST:
			...
			ITEMS_DBG(KERN_ERR "items: exist: key=%s, ret=%d\n", qt.key, ret);
			return ret;
		case ITEMS_INTEGER:
			ret = item_integer(qt.key, qt.section);
			ITEMS_DBG(KERN_ERR "items: integer: key=%s, ret=%d\n", qt.key, ret);
			return ret;
		case ITEMS_EQUAL:
			ret = item_equal(qt.key, qt.value, qt.section);
			ITEMS_DBG(KERN_ERR "items: equal: key=%s, v=%s, sec=%d, ret=%d\n", qt.key, qt.value, qt.section, ret);
			return ret;
		case ITEMS_ITEM:
			ret = item_string_item(qt.fb, qt.key, qt.section);
			ITEMS_DBG(KERN_ERR "items: items: fb=%s, key=%s, sec=%d, ret=%d\n", qt.fb, qt.key, qt.section, ret);
			if(ret) goto error;
			else break ;
		case ITEMS_STRING:
			ret = item_string(qt.fb, qt.key, qt.section);
			ITEMS_DBG(KERN_ERR "items: string: fb=%s, key=%s, sec=%d, ret=%d\n", qt.fb, qt.key, qt.section, ret);
			if(ret) goto error;
			else break ;
		case ITEMS_REINIT:
			ret = copy_from_user((char *)init_vals , arg , 8);
			ret = item_init((char *)init_vals[0] , init_vals[1]);
			ITEMS_DBG(KERN_ERR "reinit items at: 0x%8x, len:0x%x\n", init_vals[0], init_vals[1]);
			if(ret) goto error;
			break;
		case ITEMS_GETTR:
			copy_to_user((char *)arg, items_dtra, 0x1000);
			return 0;
		default:
			goto error;
	}
...
{
//	ITEMS_DBG(KERN_ERR "items opened.\n");
    /* reset export pos */
    item_export(NULL, 0);
    return 0;
}
   ...
   char __user *buf, size_t length, loff_t *offset)
{
	...
	ITEMS_DBG(KERN_ERR "prepare function to GRAM: Gx%p, Vx%p\n", ddf, __ddf);
	memcpy((void *)ddf, (void *)__ddf, 0x2000);
	ITEMS_DBG(KERN_ERR "reset DDR to 333M\n");
...
static int __init items_dev_init(void)
{
	...
	if(!items_dtra)
	    ...
	    void *__b = phys_to_virt(RESERVED_BASE_DTRA);
