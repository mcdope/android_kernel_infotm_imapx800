...
#define IMAP_BATT_DEBUG
	...
	#define BATT_DBG(x...) do{\
	    if(item_exist("batt.debug"))\
	    	if(item_equal("batt.debug","1", 0))\
	    		printk(KERN_ERR "[imapx_battery_a]" x);\
	}while(0)
#else
...
enum imap_batt_current_level_e{
         BATT_CURRENT_LEVEL_0 = 0,//no run
         BATT_CURRENT_LEVEL_1,
         BATT_CURRENT_LEVEL_2,
         BATT_CURRENT_LEVEL_3,
         BATT_CURRENT_LEVEL_4,
	 BATT_CURRENT_LEVEL_5,
};
...
extern void rtc_info_set(int num, int val);
extern int rtc_info_get(int num);
...
extern int imap_unexpect_shut(void);
...
extern int batt_export_1180_a_cap;
extern int batt_export_1180_a_vol;
...
static void battery_debug_disp(void)
{
	...
	else if(batt_info.charger_state == BATT_CHARGER_AC)
		BATT_DBG("batt_info.charger_state == BATT_CHARGER_AC\n");
	else if(batt_info.charger_state == BATT_CHARGER_USB)
		BATT_DBG("batt_info.charger_state == BATT_CHARGER_USB\n");
	else
		BATT_DBG("batt_info.charger_state == UNKNOW\n");
}
#else
static void battery_debug_disp(void){}
#endif
...
static int imap_batt_chg_batt_isfull(void)
{
	...
	if(isacon())
	{
		...
		if(item_exist("chargerfull.detect"))
		{
			...
			pin_val = (index>0)? imapx_pad_get_indat(abs_index) : (!imapx_pad_get_indat(abs_index));
	...
	BATT_DBG("%s, retutn is %d\n", __func__, tmp);
	return tmp;
#endif
}

static void imap_batt_sta_pin_cfg(void)
{
	int index;
	int abs_index;
...
static void imap_set_last_batt_cap(int value)
{
	...
	{
		reg_data = 0x80|value;
		rtc_info_set(7, reg_data);
		reg_data = rtc_info_get(7);
		BATT_DBG("+++ INFO 7 +++ is %d\n", reg_data);
	}
}
...
static int imap_get_batt_cap(int batt_v)
{
	...
	if(batt_info.batt_state == POWER_SUPPLY_STATUS_DISCHARGING)
	{
	...
	if(batt_info.batt_init_flag == 1)
	{
		...
		if(batt_info.charger_state != BATT_CHARGER_OFF)
		{
			...
			{	
				...
				else if(cap_new < cap_disp)
				{	
					if(batt_info.batt_update_flag)
					{	
						batt_info.batt_update_flag = 0;
						if(batt_info.batt_voltage < batt_info.batt_old_voltage)
						{
							batt_info.batt_old_voltage = batt_info.batt_voltage;
...
{	
	switch(psp){
		...
		case POWER_SUPPLY_PROP_PRESENT:
			if(batt_info.charger_state == BATT_CHARGER_AC)
				val->intval = 1;
			else
				val->intval = 0;
			battery_debug_disp();
...
static int imap_usb_get_prop(struct power_supply *psy,
		 enum power_supply_property psp,
		 union power_supply_propval *val)
{
 ...
 switch(psp){ 
	case POWER_SUPPLY_PROP_ONLINE:
	case POWER_SUPPLY_PROP_PRESENT:
		if(batt_info.charger_state == BATT_CHARGER_USB)
			val->intval = 1;
		else
			val->intval = 0;
		battery_debug_disp();
...
static enum power_supply_property imap_usb_props[] = {
	POWER_SUPPLY_PROP_ONLINE,
	POWER_SUPPLY_PROP_PRESENT,
	POWER_SUPPLY_PROP_VOLTAGE_NOW,
	POWER_SUPPLY_PROP_CURRENT_NOW,
...
static struct power_supply imap_psy_usb = {
	.name = "usb",
	.type = POWER_SUPPLY_TYPE_USB,
	.properties =imap_usb_props,
	.num_properties = ARRAY_SIZE(imap_usb_props),
	.get_property = imap_usb_get_prop,
};
	...
	switch(psp){
		...
		case POWER_SUPPLY_PROP_CAPACITY:
			...
			else
			{
				if(batt_info.batt_init_flag == 0)
				{
				    	batt_export_1180_a_cap = batt_info.batt_old_capacity;
				...
				else if(batt_info.batt_init_flag == 5)
				{
					...
					batt_export_1180_a_cap = batt_info.batt_capacity;
...
static void imap_charger_mon(void)
{
	...
	{//usb/ac/unknown/host
		...
		if(tmp == 0)
		{
			...
			power_supply_changed(&imap_psy_usb);
			power_supply_changed(&imap_psy_batt);
		}
		else
		{
			usb_state = get_otg_charge_mode();
			if(usb_state == BATT_OTG_AC)
			{
			    	if(batt_info.charger_state != BATT_CHARGER_AC)
				{
					BATT_DBG("batt is charge use ac\n");	
					batt_info.charger_state = BATT_CHARGER_AC;
					batt_info.batt_state = POWER_SUPPLY_STATUS_CHARGING;
					power_supply_changed(&imap_psy_ac);
				}
			}
			else if(usb_state == BATT_OTG_USB)
			{
			    	if(batt_info.charger_state != BATT_CHARGER_USB)
				{
				    	BATT_DBG("batt is charger use usb\n");
					batt_info.charger_state = BATT_CHARGER_USB;
					batt_info.batt_state = POWER_SUPPLY_STATUS_CHARGING;
					power_supply_changed(&imap_psy_usb);
				}
			}
			else if(usb_state == BATT_OTG_HOST)
			{
			    	if(batt_info.charger_state != BATT_CHARGER_OFF)
				{
				    	BATT_DBG("batt is host\n");
					batt_info.charger_state = BATT_CHARGER_OFF;
					batt_info.batt_state = POWER_SUPPLY_STATUS_DISCHARGING;
					power_supply_changed(&imap_psy_usb);
					power_supply_changed(&imap_psy_ac);
			...
			{
			    	BATT_DBG("batt is charger use known\n");
				batt_info.charger_state = BATT_CHARGER_UNKNOW;
				batt_info.batt_state = POWER_SUPPLY_STATUS_CHARGING;
			}
...
static void imap_battery_mon_update(void)
{
	...
	if(item_exist("batt.curvetest"))
		if(item_equal("batt.curvetest", "1", 0))
...
static int imap_battery_mon(void)
{
...
static void imap_battery_item_curve (void)
{
	...
	{
		//BATT_DBG("batt.chg is exist\n");
		//BATT_DBG("batt.chg tbl is \n");
		for (i = 0;i < 12;i++)
		{
			charge_curve[i].vol = item_integer("batt.chg", i);
	...
	{
		//BATT_DBG("batt.dischg is exist\n");
		//BATT_DBG("batt.dischg tbl is \n");
		for (i = 0;i < 12;i++)
		{
			discharge_curve[i].vol = item_integer("batt.dischg", i);
    ...
    {
        batt_info.batt_icc = 2000;
    }
}

static void imap_battery_get_first_voltage(void)
{
	int i, reg_data, usb_state;
	int batt_v[4], batt_ave, cap_new, cap_priv, delt_cap;
	...
	batt_info.batt_voltage = batt_ave;
	batt_info.batt_old_voltage = batt_info.batt_voltage;
	if(batt_info.batt_state == POWER_SUPPLY_STATUS_DISCHARGING)
	{
...
static void imap_battery_get_initialization(void)
{
	...
	if(isacon())
	{
		...
		{
			BATT_DBG("Unknown Charger is on\n");
			batt_info.charger_state = BATT_CHARGER_UNKNOW;
			batt_info.batt_state = POWER_SUPPLY_STATUS_CHARGING;
		}
		battery_debug_disp();
...
int bat_a_pcbtest_voltage;

int imap_battery_a_pcbtest_get_voltage(void)
{
     int adc_pcb;
     adc_pcb = batt_get_adc_val();
     bat_a_pcbtest_voltage = (int)(adc_pcb * 5000/1024);
     return bat_a_pcbtest_voltage;
}
 
int imap_battery_a_pcbtest_get_cap(void)
{
     int bat_cap;

     if(isacon())
     {
         bat_cap = imap_get_batt_v2cap(bat_a_pcbtest_voltage, charge_curve, sizeof(charge_curve)/sizeof(struct battery_curve));
     }
     else
     {
         bat_cap = imap_get_batt_v2cap(bat_a_pcbtest_voltage, discharge_curve, sizeof(discharge_curve)/sizeof(struct battery_curve));
     }
...
static int __devinit imap_battery_probe(struct platform_device *pdev)
{
	int ret = 0;
	...
	batt_info.imap_psy_usb = &imap_psy_usb;
	...
	ret = power_supply_register(&pdev->dev, batt_info.imap_psy_usb);
	if(ret)
	{
		goto usb_psy_failed;
	}

	init_MUTEX(&batt_info.mon_sem);
	batt_info.monitor_battery = imap_battery_mon;
	batt_info.mon_battery_interval = IMAP_MON_INTVAL;
	batt_mon_task = kthread_run(imap_battery_mon, NULL, "IMAP BATTERY A MONITOR");
	...
	__imapx_register_pcbtest_batt_v(imap_battery_a_pcbtest_get_voltage, imap_battery_a_pcbtest_get_cap);
...
usb_psy_failed:
	power_supply_unregister(&imap_psy_usb);
battery_psy_failed:
...
static int __devexit  imap_battery_remove(struct platform_device *pdev)
{
	BATT_DBG("%s\n", __func__);
...
static int imap_battery_resume(struct platform_device *pdev) 
{
	...
	u32 re_delt_time, bat_cap, delt_cap, new_cap, bat_icc;
	int usb_state;
	BATT_DBG("+++  %s +++\n", __func__);
	...
	if(re_charger_state == 0)
	{
		re_cap = imap_get_batt_v2cap(re_batt_v,  discharge_curve, sizeof(discharge_curve)/sizeof(struct battery_curve));
	}
	else
	{
	    	if((batt_info.charger_state == BATT_CHARGER_AC)||(batt_info.charger_state == BATT_CHARGER_USB))
	...
	if(batt_info.batt_state == POWER_SUPPLY_STATUS_CHARGING)
	{
	    	if(item_exist("batt.cap"))
		{
		    	bat_cap = item_integer("batt.cap", 0);
		}
		else
		{
		    	bat_cap = 3000;
		...
		usb_state = batt_info.charger_state;
		
		if(usb_state == BATT_OTG_AC)
		{
		    	bat_icc = batt_info.batt_icc;
		}
		else if(usb_state == BATT_OTG_USB)
		{
		    	bat_icc = 500;
...
static struct platform_driver imap_battery_driver = {
	...
	.remove = __devexit_p(imap_battery_remove),
...
static int __init imap_bat_init(void)
{
	...
	if(item_exist("pmu.model")) {
        	if(item_equal("pmu.model", "axp202", 0)) {
            		BATT_DBG("********** PMU is axp202, iMAPX820_BATTERY_A Driver Not install **********\n");
            		return 0;
        	}
    	}
	...
	{
	    	if(item_equal("charger.pattern", "noFGmode2", 0))
		{
		    	BATT_DBG("Success: imapx820_battery_a noFGmode2 check OK\n");
		}
		else
		{
		    	BATT_DBG("imapx820_battery_a is not install\n");
			return 0;
		}
	}
	else
	...
	{
		BATT_DBG("********** iMAPX820_BATTERY_A Driver Not install**********\n");
		return 0;
	}
	
	BATT_DBG("********** iMAPX820_BATTERY_a Driver **********\n");
	BATT_DBG("%s\n", __func__);
