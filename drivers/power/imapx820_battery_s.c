...
{	
	...
	switch(psp){
		...
		case POWER_SUPPLY_PROP_CAPACITY:
			...
			{
				BAT_DBG("testing batt curvetest ......  please wait!\n"); 
				BAT_DBG("[charger state] = %d, [batt state] = %d\n", batt_s_info.charger_state, batt_s_info.batt_state);
				val->intval = 100;
			}
			else
			{
				...
				else
				{
					...
					if(item_exist("batt.debug"))
					{
						if(item_equal("batt.debug", "1", 0))
						{
							BAT_DBG("battery voltage is [%d], display_capacity is [%d]\n",\
...
static void imap_battery_s_mon_update(void)
{
	...
	if(item_exist("batt.curvetest"))
		if(item_equal("batt.curvetest", "1", 0))
			BAT_DBG(" ****** adc = [%d],    battery = [%d] ****** for test only\n", adc_temp, batt_val_temp);
	count ++;
...
static int imap_battery_s_mon(void)
{
...
static void imap_battery_s_item_curve (void)
{
	...
	{
		BAT_DBG("batt.chg is exist\n");
		BAT_DBG("batt.chg tbl is \n");
		for (i = 0;i < 12;i++)
		{
	...
	{
		BAT_DBG("batt.dischg is exist\n");
		BAT_DBG("batt.dischg tbl is \n");
		for (i = 0;i < 12;i++)
		{
...
static void imap_battery_s_get_first_voltage(void)
{
	...
	u32 delt_time;
	int batt_pwroff_priv, batt_pwroff;
...
static void imap_battery_s_get_initialization(void)
{
	...
	BAT_DBG("%s, batt_s_info.batt_old_capacity is %d\n", batt_s_info.batt_old_capacity);
}

static struct task_struct *batt_s_mon_task;
static int __devinit imap_battery_s_probe(struct platform_device *pdev)
{
	int ret = 0;
	...
	init_MUTEX(&batt_s_info.mon_sem);
	batt_s_info.monitor_battery = imap_battery_s_mon;
...
static int __devexit  imap_battery_s_remove(struct platform_device *pdev)
{
	BAT_DBG("%s\n", __func__);
...
{
	int re_batt_v, re_cap, re_charger_state, re_batt_full, re_delt_cap;
	u32 re_delt_time;
...
static struct platform_driver imap_battery_s_driver = {
	...
	.remove = __devexit_p(imap_battery_s_remove),
...
static int __init imap_battery_s_init(void)
{
    ...
    if(item_exist("pmu.model")) {
        if(item_equal("pmu.model", "axp202", 0)) {
            BAT_DBG("********** PMU is axp202, iMAPX820_BATTERY_S Driver Not install **********\n");
            return 0;
        }
    }
