...
static void axp_check_vbus_high(struct delayed_work *work)
{
    ...
    if(ret) {
        ...
        axp_write(charger->master, 0x33, 0xc9);/*set charging current 1200mA*/
        axp_change(charger);
        schedule_delayed_work(&charger->check_vbus_low,
                                msecs_to_jiffies(100));
    }else {
...
static void axp_check_ac_work(struct delayed_work *work)
{
	struct axp_charger *charger =
		container_of(work, struct axp_charger, check_ac_work);
	...
	if(charger->ac_valid) {
		if(ac_state == 0) {
			...
			DBG_PSY_MSG("AC IN: set charging current 600mA\n");
			axp_change(charger);
		}
	}else {
		if(ac_state == 1) {
...
static void axp_check_usb_work(struct delayed_work *work)
{
	int state;
	struct axp_charger *charger =
		container_of(work, struct axp_charger, check_usb_work);
	...
	case 0:/*idle*/
		if(charger->flag != 0) {
		    	if(charger->flag == 1) {
			    DBG_PSY_MSG("--- AC CHARGER OUT ---\n");
			    if(charger->ac_valid == 1) {
				axp_update(charger->master, 0x30, 0x1, 0x3);/*vbus limit 500mA*/
				axp_write(charger->master, 0x33, 0xc4);/*set charging current 600mA*/
				DBG_PSY_MSG("AC CHARGER OUT: but AC IN, vbus limit 500mA, set current 600mA\n");
				charger->flag = 0;
			    }else {
				axp_update(charger->master, 0x30, 0x1, 0x3);/*vbus limit 500mA*/
				axp_write(charger->master, 0x33, 0xc2);/*set charging current 500mA*/
				DBG_PSY_MSG("AC CHARGER OUT: and AC OUT, vbus limit 500mA, set current 500mA\n");
			    }
			    axp_change(charger);
			    charger->flag = 0;
				...
				axp_update(charger->master, 0x30, 0x1, 0x3);/*vbus limit 500mA*/
				axp_write(charger->master, 0x33, 0xc4);/*set charging current 600mA*/
				DBG_PSY_MSG("USB CHARGER OUT: but AC IN, vbus limit 500mA, set current 600mA\n");
			    }else {
				axp_update(charger->master, 0x30, 0x1, 0x3);/*vbus limit 500mA*/
				axp_write(charger->master, 0x33, 0xc2);/*set charging current 500mA*/
				DBG_PSY_MSG("USB CHARGER OUT: and AC OUT, vbus limit 500mA, set current 500mA\n");
			    }
			    axp_change(charger);
			    charger->flag = 0;
			    schedule_delayed_work(&charger->check_usb_work, msecs_to_jiffies(500));
				...
				schedule_delayed_work(&charger->check_usb_work, msecs_to_jiffies(0));
				schedule_delayed_work(&charger->workstate, msecs_to_jiffies(500));
			}
		}else
		    schedule_delayed_work(&charger->check_usb_work, msecs_to_jiffies(500));
		break;
	...
	case 1:/*ac charger*/
		if(charger->flag != 1) {
		    DBG_PSY_MSG("+++ AC CHAGER IN +++");
		    charger->flag = 1;
		    axp_update(charger->master, 0x30, 0x3, 0x3);/*vbus no limit*/
		    axp_write(charger->master, 0x33, 0xc4);/*set charging current 600mA*/
		    DBG_PSY_MSG("AC CHAGER IN: vbus no limit, set current 1200mA\n");
		    axp_change(charger);
		}
		schedule_delayed_work(&charger->check_usb_work, msecs_to_jiffies(500));
		break;
	...
	case 2:/*usb charger*/
		if(charger->flag != 2) {
			...
			if(charger->ac_valid == 1) {
			    axp_write(charger->master, 0x33, 0xc4);/*set charging current 600mA*/
			    DBG_PSY_MSG("USB CHARGER IN: and AC IN, vbus limit 500mA, set current 600mA\n");
			}else {
                charger_ing =0;
			    axp_write(charger->master, 0x33, 0xc2);/*set charging current 500mA*/
			    DBG_PSY_MSG("USB CHARGER IN: and AC OUT, vbus limit 500mA, set current 500mA\n");
			}
			axp_change(charger);
		}
		schedule_delayed_work(&charger->check_usb_work, msecs_to_jiffies(500));
