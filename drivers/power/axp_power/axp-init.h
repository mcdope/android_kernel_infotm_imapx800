#ifndef __AXP_INIT_H__
#define __AXP_INIT_H__
...
//#if defined (CONFIG_AXP_CHGCURSET)
	...
	if(ret)
		return ret;
	DBG_PSY_MSG("charging current = %d mA\n", cur);
//#if defined (CONFIG_KP_AXP20)
	if(cur >= 300 && cur <= 1800){
		tmp = (cur -201)/100;
	    DBG_PSY_MSG("charging tmp -- = %x \n", tmp);
...
static int axp_set_charge(struct axp_charger *charger)
{
	...
	if(ret){
		DBG_PSY_MSG("[AXP]error in set charge voltage\n");
		return ret;
	}
	/* 充电电流 */
	ret = axp_set_chargecurrent(charger->chgcur);
	if(ret){
		DBG_PSY_MSG("[AXP]error in set charge current\n");
		return ret;
	}
	/* 结束充电 */
	ret = axp_set_chargeendrate(charger->chgend);
	if(ret){
		DBG_PSY_MSG("[AXP]error in set charge end current rate\n");
		return ret;
	}
	/* 预充电时间设置 */
	ret = axp_set_prechargetime(charger->chgpretime);
	if(ret){
		DBG_PSY_MSG("[AXP]error in set precharge time\n");
		return ret;
	}	
	/* 恒流充电设置 */
	ret = axp_set_cstchargetime(charger->chgcsttime);
	if(ret){
		DBG_PSY_MSG("[AXP]error in set constant charge time\n");
	}
...
static int axp_battery_adc_set(struct axp_charger *charger)
{
	...
	if (ret){
		DBG_PSY_MSG("[AXP]Unable to enable adc\n");
		return ret;
	}
	/* 设置adc采样率 */
	ret = axp_set_freq(charger->adc_freq);
	if(ret){
		DBG_PSY_MSG("[AXP]Unable to set adc frequency\n");
	}
	return ret;
}
...
static int axp_set_pekinfo(struct platform_device *pdev)
{
	...
	if(ret) {
		DBG_PSY_MSG("[AXP]Unable to Register the power key\n");
	}
#endif
	return ret;
}
...
static int axp_set_pekopen(int pek)
{
	...
	if(pek >=128 && pek <=3000){	
		...
		if(ret)
			DBG_PSY_MSG("[AXP]Unabel to set pek open time\n");
	}
#endif
...
static int axp_set_peklong(int pek)
{
	...
	if(pek >=1000 && pek <=2500){
		...
		if(ret){
			DBG_PSY_MSG("[AXP]Unabel to set pek long time\n");
		}
	}
#endif
	return ret;
...
static int axp_set_pekdelay(int pek)
{
	...
	if(ret){
		DBG_PSY_MSG("[AXP]Unabel to set pek powerok delay time\n");
	}
#endif
	return ret;
}
...
static int axp_set_pekoff(int pek)
{
	...
	if(pek >=4000 && pek <=10000){	
		...
		if(ret){
			DBG_PSY_MSG("[AXP]Unabel to set pek power off time\n");
		}
	}
#endif
	return ret;
...
static int axp_set_pek(void)
{
	...
	if(ret){
		DBG_PSY_MSG("[AXP]Unable to set pek startup\n");
		return ret;
	}
	/* 长按时间 */
	ret = axp_set_peklong(PEKLONG);
	if(ret){
		DBG_PSY_MSG("[AXP]Unable to set longtime pek\n");
		return ret;
	}
	/* 长按关机使能 */
	ret = axp_set_pekoffen(PEKOFFEN);
	if(ret){
		DBG_PSY_MSG("[AXP]Unable to set pekoff\n");
		return ret;
	}
	/* power ok 信号延迟*/
	ret = axp_set_pekdelay(PEKDELAY);
	if(ret){
		DBG_PSY_MSG("[AXP]Unable to set power ok delay\n");
		return ret;
	}
	/* 长按pmu关机时间 */
...
static int axp_set_noe(int noe)
{
	...
	if(noe >=128 && noe <=3000){	
		...
		if(ret){
			DBG_PSY_MSG("[AXP]Unabel to set n_oe power off time\n");
		}
	}
#endif
	return ret;
...
static int axp_set_otpoffen(int otp)
{
	...
	if(ret){
		DBG_PSY_MSG("[AXP]Unabel to enable over temperture power off function\n");
	}
#endif
	return ret;
}
...
static int axp_set_opclinit(void)
{
	...
	ret = axp_set_pek();
	if(ret){
		DBG_PSY_MSG("[AXP]Unable to set pek property\n");
		return ret;
	}
	/* 设置N_OE开关属性 */
	ret = axp_set_noe(NOEOFF);
	if(ret){
		DBG_PSY_MSG("[AXP]Unable to set noe\n");
		return ret;
	}
	/* 设置过温关机使能*/
	ret = axp_set_otpoffen(OTPOFFEN);
	if(ret){
		DBG_PSY_MSG("[AXP]Unable to set over temperture power off\n");
	}
	return ret;
}
...
#if defined (CONFIG_KP_AXP20)
	uint8_t ocv_cap[31];
    if(item_exist("batt.ocv")) {
	    ocv_cap[0]  = item_integer("batt.ocv", 0);
	    ocv_cap[1]  = 0xC1;
	    ocv_cap[2]  = item_integer("batt.ocv", 1);
	    ocv_cap[3]  = 0xC2;
	    ocv_cap[4]  = item_integer("batt.ocv", 2);
	    ocv_cap[5]  = 0xC3;
	    ocv_cap[6]  = item_integer("batt.ocv", 3);
	    ocv_cap[7]  = 0xC4;
	    ocv_cap[8]  = item_integer("batt.ocv", 4);
	    ocv_cap[9]  = 0xC5;
	    ocv_cap[10] = item_integer("batt.ocv", 5);
	    ocv_cap[11] = 0xC6;
	    ocv_cap[12] = item_integer("batt.ocv", 6);
	    ocv_cap[13] = 0xC7;
	    ocv_cap[14] = item_integer("batt.ocv", 7);
	    ocv_cap[15] = 0xC8;
	    ocv_cap[16] = item_integer("batt.ocv", 8);
	    ocv_cap[17] = 0xC9;
	    ocv_cap[18] = item_integer("batt.ocv", 9);
	    ocv_cap[19] = 0xCA;
	    ocv_cap[20] = item_integer("batt.ocv", 10);
	    ocv_cap[21] = 0xCB;
	    ocv_cap[22] = item_integer("batt.ocv", 11);
	    ocv_cap[23] = 0xCC;
	    ocv_cap[24] = item_integer("batt.ocv", 12);
	    ocv_cap[25] = 0xCD;
	    ocv_cap[26] = item_integer("batt.ocv", 13);
	    ocv_cap[27] = 0xCE;
	    ocv_cap[28] = item_integer("batt.ocv", 14);
	    ocv_cap[29] = 0xCF;
	    ocv_cap[30] = item_integer("batt.ocv", 15);
    }else {
        ocv_cap[0]  = OCVREG0;
        ocv_cap[1]  = 0xC1;
        ocv_cap[2]  = OCVREG1;
...
static int axp_set_warning(int warn1,int warn2)
{
	...
	if(ret){
		DBG_PSY_MSG("[AXP]Unable to set warning voltage 1\n");
		return ret;
	}
...
static int axp_usb_vollim(int vol)
{
	...
	if(vol){
		...
		if(vol >= 4000 && vol <=4700){
			...
			if(ret)
				DBG_PSY_MSG("[AXP]Unable to set usb voltage set %d mV\n",val);
		}
	}
	else{
		ret = axp_clr_bits(axpcharger->master, AXP_CHARGE_VBUS, 0x40);	
		if(ret)
			DBG_PSY_MSG("[AXP]Unable to set usb voltage set\n");
	}	
#endif
	return ret;	
}
...
static int axp_usb_curlim(int cur)
{
	...
	if(cur){
		...
		if(cur == 900){
			...
			if(ret)
				DBG_PSY_MSG("[AXP]Uable to set usb current set 900mA\n");
    	}
		else if (cur == 500){
			ret = axp_update(axpcharger->master, AXP_CHARGE_VBUS,0x01, 0x03);
			if(ret)
				DBG_PSY_MSG("[AXP]Uable to set usb current set 500mA\n");
		}
		else if (cur == 100){
			ret = axp_update(axpcharger->master, AXP_CHARGE_VBUS,0x10, 0x03);
			if(ret)
				DBG_PSY_MSG("[AXP]Uable to set usb current set 100mA\n");
		}
	}
	else
		ret = axp_set_bits(axpcharger->master, AXP_CHARGE_VBUS, 0x03);
		if(ret)
			DBG_PSY_MSG("[AXP]Uable to set usb current set 0mA\n");
#endif
...
static int axp_battery_first_init(struct axp_charger *charger)
{
	...
	if(ret){
		DBG_PSY_MSG("[AXP]Unable to enable adc nor set adc frequency\n");
		return ret;
	}
	/* 设置充电参数 */
	ret = axp_set_charge(charger);
	if(ret){
		DBG_PSY_MSG("[AXP]Uable to set charger parameters\n");
		return ret;
	}
	/* 设置开关机参数 */
	ret = axp_set_opclinit();
	if(ret){
		DBG_PSY_MSG("[AXP]Unable to set pek/noe/otp \n");
		return ret;
	}
	/* 设置ocv对应剩余电量表 */
	ret = axp_set_ocv();
	if(ret){
		DBG_PSY_MSG("[AXP]Unable to set ocv/capicaty\n");
		return ret;
	}
	/* 设置低电唤醒警告电压 */
	ret = axp_set_warning(WARNVOL1,WARNVOL2);
	if(ret){
		DBG_PSY_MSG("[AXP]Unable to set warning voltage\n");
		return ret;
	}
	/* 设置电池内阻的默认值 */
	ret = axp_set_rdcclose();
	if(ret){
		DBG_PSY_MSG("[AXP]Unable to close rdc calculate\n");
		return ret;
	}
	/* irq pin 唤醒使能和16s长按ldo1关闭使能 */
	ret = axp_set_bits(charger->master,AXP_HOTOVER_CTL,0x08);
	if(ret){
		DBG_PSY_MSG("[AXP]Unable to enable irq wake up and close ldo1\n");
		return ret;
	}
	/* usb 限压 */
	ret = axp_usb_vollim(USBVOLLIM);
	if(ret){
		DBG_PSY_MSG("[AXP]Unable to set usb voltage limite\n");
		return ret;
	}
	/* usb 限流 */
	ret = axp_usb_curlim(USBCURLIM);
	if(ret)
		DBG_PSY_MSG("[AXP]Unable to set usb current limite\n");
	return ret;
}
...
static int axp_get_rdc(struct axp_charger *charger)
{
	...
	rdc = v[1]*2;
#endif
	DBG_PSY_MSG("%s->%d,calculate rdc: rdc = %d\n",__FUNCTION__,__LINE__,rdc);
