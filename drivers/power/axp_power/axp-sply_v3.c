...
#if 0
static void drvbus_pull(int val)
{
...
static int get_id_pin_state(void)
{
    ...
    id_index = item_integer("otg.id", 1);
    ret = imapx_pad_get_indat(id_index);

    return ret;
}

static void axp_check_id_low(struct delayed_work *work)
{
    struct axp_charger *charger =
            container_of(work, struct axp_charger, check_id_low);
...
static void axp_check_id_high(struct delayed_work *work)
{
    struct axp_charger *charger =
            container_of(work, struct axp_charger, check_id_high);
    int ret = get_id_pin_state();
...
{
    if(item_exist("charger.pattern") == 0 ||
            item_equal("charger.pattern", "mode2", 0)) {
