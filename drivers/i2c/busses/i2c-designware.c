...
static int imapx800_i2c_module_init(int num)
{
    int ret = -1;
    switch(num)
    {
	case 0:
	    ret = imapx_pad_cfg(IMAPX_I2C0);
	    break;
	case 1:
	    ret = imapx_pad_cfg(IMAPX_I2C1);
	    break;
	case 2:
	    ret = imapx_pad_cfg(IMAPX_I2C2);
	    break;
	case 3:
	    ret = imapx_pad_cfg(IMAPX_I2C3);
	    break;
	case 4:
	    ret = imapx_pad_cfg(IMAPX_I2C4);
	    break;
	case 5:
	    ret = imapx_pad_cfg(IMAPX_I2C5);
	    break;
    }
    return ret;
}
...
static int __devinit dw_i2c_probe(struct platform_device *pdev)
{
	...
	dev_dbg(dev->dev, "dw_i2c_probe, irq=%d mem=0x%x\n", irq, mem->start);
	if(imapx800_i2c_module_init(pdev->id) == -1)
	{
	   printk("imapx800 i2c module init failed, please sure case configure\n"); 
	   r = -EINVAL;
	   goto err_free_mem;
	}
