 ...
 *
 *  Copyright (C) 2011~2012 Lex Hsieh / sensortek <lex_hsieh@sensortek.com.tw>
 *
...
#define SENSORS_STK8312
//#define STK_ALLWINNER_PLATFORM
#define STK_ACC_DRIVER_VERSION	"1.5.8 revised 20130304"
...
#if 0 //-----------------add by zhanbing.li start---------------
#ifndef STK_ALLWINNER_PLATFORM
	#ifdef SENSORS_STK8313
		#include <linux/stk8313.h>
	#elif defined SENSORS_STK8312
...
static int suspend_en = 0;
static char offset_reset[3] = {0};	
...
static int STK831x_ReportValue(struct stk831x_data *stk)
{
	...
	input_report_abs(stk->input_dev, ABS_Y, -stk->raw_data[0]);
	...
	if(stk->first_enable)
	{
		...
		if ((stk_get_file_content(r_buf, STK_ACC_CALI_FILE_SIZE)) == 0)
		{
			if(r_buf[0] == STK_ACC_CALI_VER0 && r_buf[1] == STK_ACC_CALI_VER1)
			{
				...
				msleep(1);
				STK831x_GetOffset(offset_reset);
...
static int STK831X_VerifyCali(struct stk831x_data *stk, unsigned char en_dis)
{
	...
	int acc_ave[3] = {0, 0, 0};
	const unsigned char verify_sample_no = 2;		
#ifdef SENSORS_STK8313
...
static int STK831x_SetCali(struct stk831x_data *stk, char sstate)
{
	...
	if(store_location == 1)
	{
		...
		{
			result = stk_store_in_file(new_offset);
			if(result)
			{
	...
	if(status != STK_K_RUNNING)
	{
		if ((stk_get_file_content(r_buf, STK_ACC_CALI_FILE_SIZE)) == 0)
		{
			if(r_buf[0] == STK_ACC_CALI_VER0 && r_buf[1] == STK_ACC_CALI_VER1)
			{
				...
				offset[1] = r_buf[3];
				offset[2] = r_buf[4];
				printk(KERN_INFO "%s:file offset:%#02x,%#02x,%#02x\n", __func__, offset[0], offset[1], offset[2]);
			}
			else
			{
				printk(KERN_ERR "%s: cali version number error! r_buf=0x%x,0x%x,0x%x,0x%x,0x%x\n", 
...
static DEVICE_ATTR(recv, 0222, NULL, stk831x_recv_store);
...
static int stk831x_suspend(struct device *dev)
{
	...
	{
		suspend_en = 1;
	//	printk(KERN_INFO "%s STK831x_SetEnable(0), suspend_en=%d\n", __func__, suspend_en);
		STK831x_SetEnable(stk, 0);
...
static int stk831x_resume(struct device *dev)
{
	...
	if (error) 
	{		
		printk(KERN_INFO "%s:stk831x initialization failed\n", __func__);	
	}
	STK831x_SetOffset(offset_reset);
//	if(atomic_read(&stk->enabled))	
	if(suspend_en==1)
	{
		suspend_en = 0;
		printk(KERN_INFO "%s STK831x_SetEnable(1), suspend_en=%d\n", __func__, suspend_en);
		STK831x_SetEnable(stk, 1);
	}
	return 0;		
}
...
/**************detect gsensor i2c, add by zhanbing.li 2013-01-28*********/
...
static int __init stk831x_init(void)
{
	...
	//if(item_exist("sensor.grivaty") && item_equal("sensor.grivaty.model", "stk8312", 0)) 
	{
