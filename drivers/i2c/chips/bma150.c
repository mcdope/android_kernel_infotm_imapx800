		...
		struct bma150acc *acc)
{
	...
	const char *convert_indicator = CONFIG_ACC_BMA023_CONVERT;
	...
	if (client == NULL) {
		...
		if(strnlen(convert_indicator,4) < 3) ;
		else {
			short d[3], s[3], c[3], i;
			d[0] = acc->x; d[1] = acc->y; d[2] = acc->z;
			for(i = 0; i < 3; i++ ){
				s[i] = ((c[i] = convert_indicator[i]) >97) ? 1:
					(c[i] += 32, -1);
				c[i] -= 'x';
			}
			acc->x = d[c[0]] * s[0];
...
static int __init BMA150_init(void)
{
	...
	adapter = i2c_get_adapter(CONFIG_ACC_BMA023_I2C + 1);
	if (!adapter) {
