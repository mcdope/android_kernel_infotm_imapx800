...
atomic_t delay;
atomic_t enable;
void gsensor_write_offset_to_file_d06(void);
void gsensor_read_offset_from_file_d06(void);
char OffsetFileName_06[] = "/data/misc/dmt/offset.txt"; 
//**************************************************
...
void device_i2c_read_xyz_d06(struct i2c_client *client, s8 *xyz);
...
static struct device_attribute DMT_attributes[] = {
	__ATTR(enable_acc, /*0660*/0666, DMT_enable_acc_show, DMT_enable_acc_store),
	__ATTR(delay_acc,  /*0660*/0666, DMT_delay_acc_show,  DMT_delay_acc_store),
	__ATTR_NULL,
};
...
int gsensor_read_accel_avg_d06(raw_data *avg_p )
{
	...
	for(i = 0; i < AVG_NUM; i++) 
	{      
		device_i2c_read_xyz_d06(dev.client, (s8 *)&xyz);
...
/* calc delta offset */
int gsensor_calculate_offset_d06(int gAxis,raw_data avg)
{
...
void gsensor_calibrate_d06(int side)
{
	raw_data avg;
	// get acceleration average reading
	gsensor_read_accel_avg_d06(&avg);
	// calculate and set the offset
	gsensor_calculate_offset_d06(side, avg); 
}

int gsensor_reset_d06(struct i2c_client *client)
{
...
void gsensor_set_offset_d06(int val[3])
{
	int i;
	for(i = 0; i < SENSOR_DATA_SIZE; ++i)
		offset.v[i] = (s8) val[i];
...
struct file_operations dmt06_g_sensor_fops = 
{
	.owner = THIS_MODULE,
	.read = device_read,
	.write = device_write,
...
static long device_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	{
		case SENSOR_RESET:
			gsensor_reset_d06(dev.client);
			return ret;

		case SENSOR_CALIBRATION:
			// get orientation info
			if(copy_from_user(&intBuf, (int*)arg, sizeof(intBuf))) return -EFAULT;
			gsensor_calibrate_d06(intBuf[0]);
			printk("intBuf[0] %d\n",intBuf[0]);
			// save file
			gsensor_write_offset_to_file_d06();
		...
		case SENSOR_GET_OFFSET:
			// get data from file 
			gsensor_read_offset_from_file_d06();
			for(i = 0; i < SENSOR_DATA_SIZE; ++i)
				intBuf[i] = offset.v[i];
...
void device_i2c_read_xyz_d06(struct i2c_client *client, s8 *xyz_p)
{
	...
	for(i = 0; i < SENSOR_DATA_SIZE; ++i)
	{
		xyz_p[i] = 0;
		for(j = 0; j < SENSOR_DATA_SIZE; ++j)
			xyz_p[i] += sensorlayout_06[i][j] * xyzTmp[j];
	}
}
...
static int __init device_init(void)
{
	...
	//struct i2c_client *client;
	printk("Enter dmard06 init \n");
	if(item_exist("sensor.grivaty") && item_equal("sensor.grivaty.model", "dmard06", 0)) {
		IN_FUNC_MSG;
		...
		cdev_init(&dev.cdev, &dmt06_g_sensor_fops);  
...
/*
*********************************************************************************
add by chengbo start
*********************************************************************************
*/
		memset(&info, 0, sizeof(struct i2c_board_info));		
		info.addr = 0x1c;
		strlcpy(info.type, "dmard06", I2C_NAME_SIZE);
		...
		printk("%s: G-sensor is not dmard06 or exist.\n", __func__);		
		return -1;	
	}
/*
*********************************************************************************
...
// Set segment descriptor associated to kernel space
	set_fs(KERNEL_DS);
    	...
    	lfile=sys_open(OffsetFileName_06,O_WRONLY|O_CREAT, 0777);
	if (lfile < 0)
	{
		printk("sys_open %s error!!. %ld\n",OffsetFileName_06,lfile);
	}
	else
	{
...
// Set segment descriptor associated to kernel space
	...
	lfile = sys_open(OffsetFileName_06, O_RDONLY, 0);
	if (lfile < 0)
	{
		offset.v[0]=0;offset.v[1]=0;offset.v[2]=0;
//#if AUTO_CALIBRATION
		// get acceleration average reading
//		ret = gsensor_read_accel_avg_d06(&avg);
