...
static int __init lsm303dlhc_acc_init(void)
{
	...
	if (item_equal("sensor.grivaty.model", "lsm303dlhc_acc", 0))
	{
	memset(&info, 0, sizeof(struct i2c_board_info));
	info.addr = 0x19;  
	strlcpy(info.type, LSM303DLHC_ACC_DEV_NAME, I2C_NAME_SIZE);
	...
	{
		printk("%s: G-sensor is not lsm303dlhc_acc or exist.\n", __func__);
	}
}
