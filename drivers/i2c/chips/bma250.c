		...
		unsigned char reg_addr, unsigned char *data)
{
	...
	*data = dummy & 0x000000ff;
	...
	//printk("reg 0x%x read value 0x%x, dummy=0x%x\n", reg_addr, *data, dummy);
		...
		unsigned char reg_addr, unsigned char *data)
{
	...
	//printk("reg 0x%x write value 0x%x, dummy=0x%x\n", reg_addr, *data, dummy);
	if (dummy < 0)
		return -1;
	return 0;
}
...
static int gsensor_calibrate(struct i2c_client *client){
//	struct i2c_client *client = to_i2c_client(dev);
	struct bma250_data *bma250 = i2c_get_clientdata(client);
	int i;
	s16 xyz_acc[SENSOR_DATA_SIZE];
	static struct bma250acc xyz;

	memset(xyz_acc, 0, sizeof(xyz_acc));
	memset(avg, 0, sizeof(avg));
	
	for(i =0; i < AVG_NUM; i++){
		bma250_read_accel_xyz(bma250->bma250_client,(s16 *)&xyz);
		xyz_acc[0] += xyz.x;
		xyz_acc[1] += xyz.y;
		xyz_acc[2] += xyz.z;
	}
...
static void bma250_work_func(struct work_struct *work)
{
	...
	//printk("BMA250 g-sensor: x= %d, y= %d, z=%d\n", acc.x, acc.y, acc.z);

	gsensor_calibrate(bma250->bma250_client);
	acc.x = avg[0];
	acc.y = avg[1];
	acc.z = avg[2];
	msleep(2);
		...
		const struct i2c_device_id *id)
{
	...
	if(item_exist("sensor.grivaty.power")){
		...
		/*
			...
			msleep(20);
			regulator_set_voltage(regulator, 2800000, 3300000);
		*/
		}
	...
	if ((tempvalue == BMA250_CHIP_ID) || (tempvalue == BMA250_CHIP_ID1)) {
...
#if defined(BMA250_ENABLE_INT1) || defined(BMA250_ENABLE_INT2)
	gpio = __imapx_name_to_gpio(CONFIG_ACC_BMA250_INT);
	client->irq =  imapx_gpio_to_irq(gpio);;
	imapx_gpio_setcfg(gpio, IG_INPUT, IG_NORMAL);
	imapx_gpio_setirq(gpio, FILTER_MAX, IG_RSIE, 1);
...
//	int ret;
	...
	printk("****** %s run ! ******\n", __func__);
	if(item_exist("sensor.grivaty") && item_equal("sensor.grivaty.model", "bma250", 0))
	{
	...
	{
		printk("%s: G-sensor is not bma250 or exist.\n", __func__);
		return -1;
	}
}
