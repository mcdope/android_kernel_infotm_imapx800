...
void MC32X0_rbm(struct i2c_client *client, int enabled);
/*
typedef union {
	struct {
		s16	x;
...
//	struct mc32x0_data *data;
	...
	if (error)
		return error;
	if (val > MC32X0_MAX_DELAY)
		val = MC32X0_MAX_DELAY;
	atomic_set(&data->delay, (unsigned int) val);
...
static DEVICE_ATTR(delay, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH, mc32x0_delay_show, mc32x0_delay_store);
...
//	int err = 0;
	...
	if ( enable_RBM_calibration == 0)
	{
		...
		mcprintkreg("McubeID = %d,**MC32X0_ReadData : %d %d %d \n",McubeID, buffer[0],buffer[1],buffer[2]);
	}
	else if (enable_RBM_calibration == 1)
	{
...
{
	struct mc32x0_data *data = container_of((struct delayed_work *)work, struct mc32x0_data, work);
	struct acceleration accel = {0};
	unsigned long delay = msecs_to_jiffies(atomic_read(&data->delay));
	struct i2c_client *client = this_client;	


	mc32x0_measure(client, &accel);
...
//	script_parser_value_type_t type = SCIRPT_PARSER_VALUE_TYPE_STRING;
	...
	if(1 == device_used){
		...
		mc32x0_pin_hd = gpio_request_ex("gsensor_para",NULL);
		if (mc32x0_pin_hd==-1) {
			printk("mc32x0 pin request error!\n");
		}
		ret = 0;
...
//	}
	...
	atomic_set(&data->delay, MC32X0_MAX_DELAY);
	atomic_set(&data->enable, 0);
	...
	ret = mc32x0_chip_init(dev.client);
	if (ret < 0) {
...
//	gpio_set_one_pin_io_status(mc32x0_pin_hd,0,mc32x0_on_off_str);
...
static int __init mc32x0_init(void)
{
	...
	mc32x0_driver.detect = gsensor_detect;
...
//	int ret;
	...
	printk("****** %s run ! ******\n", __func__);
	if(item_exist("sensor.grivaty") && item_equal("sensor.grivaty.model", "mc32x0", 0))
	{
	...
	{
		printk(KERN_ERR "%s: G-sensor is not mc32x0 or exist.\n", __func__);
		return -1;
	}
}
