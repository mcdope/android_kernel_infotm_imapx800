...
#include "mg8697s_i7a.h"
#include "mg8697s_update_i7a.h"
...
static int ReadFWCheckSum()
{
	...
	unsigned char *p_data = mg8697s_i7a_firmware;
	#ifdef RANDY_DEBUG    
...
static int UpdateFW_2()
{
	...
	int DataLen = sizeof(mg8697s_i7a_firmware);
	unsigned char *p_data = mg8697s_i7a_firmware;
...
//	ctp_ops.ts_wakeup();
    printk(KERN_INFO "MG8697I7A BEGIN RESET++++\n");
	ctp_reset();
    printk(KERN_INFO "MG8697I7A END RESET++++\n");
	mg->dev = input_dev;
...
//	init_completion(&mg_touch_event);
	...
	if (err < 0) {
		pr_info( "mg8697i7a_probe: request irq failed\n");
		goto exit_irq_request_failed;
	}
    printk(KERN_INFO "MG8697I7A END Request IRQ++++\n");
    ...
    //if(item_exist("ts.downloader")){
        ...
        else
        {
            ...
            if(strcmp(downloader_name, "MG8697i7a") == 0){	       
...
static int __init mg_init(void)
{
    ...
    printk("------mg8697s_i7a module init------\n");
    if(item_exist("ts.model"))
    {
        if(item_equal("ts.model", "MG8697i7a", 0))
        {
	...
	return ret;
		...
		printk("%s: touchscreen is not mg8697s_i7a\n", __func__);
    }
	else
