...
static int ReadFWCheckSum()
{
	...
	unsigned char *p_data = mg8698s_firmware;
	#ifdef RANDY_DEBUG    
...
static int UpdateFW_2()
{
	...
	int DataLen = sizeof(mg8698s_firmware);
	unsigned char *p_data = mg8698s_firmware;
...
//	    read_buf[i] = read_buf[i+1]; 	
	...
	else if (read_buf[MG_MODE] ==1)
	{
		...
		//		input_report_abs(mg->dev, ABS_MT_TRACKING_ID, 1);
			...
			if(read_buf[ACTUAL_TOUCH_POINTS] >= 6) {
				mg->x =	COORD_INTERPRET(read_buf[MG_POS_X_HI6], read_buf[MG_POS_X_LOW6]);
				mg->y = COORD_INTERPRET(read_buf[MG_POS_Y_HI6], read_buf[MG_POS_Y_LOW6]);
				mg->w = read_buf[MG_CONTACT_IDS6] & 0x0F;
				mg->id = (read_buf[MG_CONTACT_IDS6]>>4)&0x0F;
		//		input_report_abs(mg->dev, ABS_MT_TRACKING_ID, 1);
				mg_mtreport(mg);
			}							

			if(read_buf[ACTUAL_TOUCH_POINTS] >= 7) {
				mg->x =	COORD_INTERPRET(read_buf[MG_POS_X_HI7], read_buf[MG_POS_X_LOW7]);
				mg->y = COORD_INTERPRET(read_buf[MG_POS_Y_HI7], read_buf[MG_POS_Y_LOW7]);
				mg->w = read_buf[MG_CONTACT_IDS7] & 0x0F;
				mg->id = (read_buf[MG_CONTACT_IDS7]>>4)&0x0F;
		//		input_report_abs(mg->dev, ABS_MT_TRACKING_ID, 1);
				mg_mtreport(mg);
			}							

			if(read_buf[ACTUAL_TOUCH_POINTS] >= 8) {
				mg->x =	COORD_INTERPRET(read_buf[MG_POS_X_HI8], read_buf[MG_POS_X_LOW8]);
				mg->y = COORD_INTERPRET(read_buf[MG_POS_Y_HI8], read_buf[MG_POS_Y_LOW8]);
				mg->w = read_buf[MG_CONTACT_IDS8] & 0x0F;
				mg->id = (read_buf[MG_CONTACT_IDS8]>>4)&0x0F;
		//		input_report_abs(mg->dev, ABS_MT_TRACKING_ID, 1);
				mg_mtreport(mg);
			}							

			if(read_buf[ACTUAL_TOUCH_POINTS] >= 9) {
				mg->x =	COORD_INTERPRET(read_buf[MG_POS_X_HI9], read_buf[MG_POS_X_LOW9]);
				mg->y = COORD_INTERPRET(read_buf[MG_POS_Y_HI9], read_buf[MG_POS_Y_LOW9]);
				mg->w = read_buf[MG_CONTACT_IDS9] & 0x0F;
				mg->id = (read_buf[MG_CONTACT_IDS9]>>4)&0x0F;
		//		input_report_abs(mg->dev, ABS_MT_TRACKING_ID, 1);
				mg_mtreport(mg);
			}							

			if(read_buf[ACTUAL_TOUCH_POINTS] >= 10) {
				mg->x =	COORD_INTERPRET(read_buf[MG_POS_X_HI10], read_buf[MG_POS_X_LOW10]);
				mg->y = COORD_INTERPRET(read_buf[MG_POS_Y_HI10], read_buf[MG_POS_Y_LOW10]);
				mg->w = read_buf[MG_CONTACT_IDS10] & 0x0F;
				mg->id = (read_buf[MG_CONTACT_IDS10]>>4)&0x0F;
		//		input_report_abs(mg->dev, ABS_MT_TRACKING_ID, 1);
				mg_mtreport(mg);
			}							
...
//	init_completion(&mg_touch_event);
	...
	if(item_exist("ts.downloader")){
		...
		else
		{
		       ...
		       if(strcmp(downloader_name, "mg8698") == 0){	       
...
static void mg8698_reset_detect(void)
{
	int index;
	index = item_integer("ts.reset", 1);
...
static mg8698_start_ping_i2c_addr()
{	
	struct i2c_adapter *adapter=NULL;
	__u16 i2cAddr=0x44;//slave addr
...
static int DoI2cScanDetectDev( void )
{
	...
	if(item_exist("ts.model")) {
		if(item_equal("ts.model", "auto", 0)) {
			...
			ret =  mg8698_start_ping_i2c_addr();
...
static int __init mg_init(void)
{
    ...
    printk("------mg8698 module init------\n");
    if(item_exist("ts.model"))
    {
        if(item_equal("ts.model", "mg8698", 0) || DoI2cScanDetectDev())
        {
	...
	return ret;
		...
		printk("%s: touchscreen is not mg8698\n", __func__);
    }
	else
