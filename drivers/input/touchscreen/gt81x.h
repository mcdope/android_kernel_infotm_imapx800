...
#define GTP_RST_PORT   item_integer("ts.reset", 1)//imapx_pad_index("cond9")///S5PV210_GPJ3(6)//S5PV210_GPH1(3)
#define GTP_INT_PORT   item_integer("ts.int", 1)//imapx_pad_index("cond20")///S5PV210_GPH1(3)
#define GTP_INT_IRQ   imapx_pad_irq_number(GTP_INT_PORT)///gpio_to_irq(GTP_INT_PORT)
#define GTP_INT_CFG   do{\
                                            imapx_pad_set_mode(0, 1, GTP_INT_PORT);\
                                            imapx_pad_irq_config(GTP_INT_PORT, INTTYPE_FALLING, FILTER_MAX);\
                      }while(0)///S3C_GPIO_SFN(0xF)//S3C_GPIO_SFN(2)

#define GTP_GPIO_AS_INPUT(pin)          do{\
											imapx_pad_set_dir(1,1,pin);\
											imapx_pad_set_pull(pin,0,0);\
                                        }while(0)
#define GTP_GPIO_AS_INT(pin)            do{\
											imapx_pad_set_mode(0, 1, pin);\
											imapx_pad_irq_config(pin, INTTYPE_FALLING, FILTER_MAX);\
                                        }while(0)
#define GTP_GPIO_GET_VALUE(pin)         imapx_pad_get_indat(pin)///gpio_get_value(pin)
#define GTP_GPIO_OUTPUT(pin,level)      do{\
                                            imapx_pad_set_dir(0,1,pin);\
                                            imapx_pad_set_outdat(level,1,pin);\
                     					 }while(0)///gpio_direction_output(pin,level)
#define GTP_GPIO_REQUEST(pin, label)    ///gpio_request(pin, label)
#define GTP_GPIO_FREE(pin)              ///gpio_free(pin)
#define GTP_GPIO_READ(pin)              imapx_pad_get_indat(pin)///gpio_get_value(pin)
