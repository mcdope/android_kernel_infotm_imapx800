...
//#include "mg8698s_5_simple.h"
//#include "mg8698s_update_712_simple.h"
#include "mg8697s.h"
#include "mg8697s_update.h"
...
static int CAP_X_CORD=1151;
static int CAP_MAX_X;
unsigned char *mg8697s_firmware;

#define MG_I2C_NAME	"mg8697s"
#define BUF_SIZE 		55
...
static int ReadFWCheckSum()
{
	...
	unsigned char *p_data = mg8697s_firmware;
	#ifdef RANDY_DEBUG    
...
static int UpdateFW_2()
{
	...
	int DataLen = (mg8697s_firmware == mg8697s_hd_firmware)?sizeof(mg8697s_hd_firmware):sizeof(mg8697s_simple_firmware);
	unsigned char *p_data = mg8697s_firmware;
...
{	
	//int DataLen ;
  // unsigned char *p_data ;
   int DataLen = sizeof(mg8697s_firmware);
	unsigned char *p_data = mg8697s_firmware;
...
//	init_completion(&mg_touch_event);
	...
	if(item_exist("ts.downloader")){
		...
		else
		{
		       ...
		       if(strcmp(downloader_name, "mg8697") == 0){	       
...
static int __init mg_init(void)
{
    ...
    printk("------mg8697 module init------\n");
    if(item_exist("ts.model"))
    {
        if(item_equal("ts.model", "mg8697", 0))
        {
            SCREEN_MAX_X = get_lcd_width();
            printk("SCREEN_MAX_X is %d\n",SCREEN_MAX_X);
            if (SCREEN_MAX_X == 1024){
                CAP_X_CORD = 1087;
                mg8697s_firmware = mg8697s_hd_firmware;
            } else 
                mg8697s_firmware = mg8697s_simple_firmware;

            CAP_MAX_X = CAP_X_CORD;
	...
	return ret;
		...
		printk("%s: touchscreen is not mg8697\n", __func__);
    }
	else
