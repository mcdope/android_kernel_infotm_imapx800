...
static void gsl_ts_async_work(struct work_struct *work);
static int gsl_ts_init_ts(struct i2c_client *client, struct gsl_ts *ts)
{
	...
	INIT_WORK(&ts->rw_async, gsl_ts_async_work);
...
static int gsl_ts_resume(struct device *dev)
{
	...
	imapx_pad_set_outdat(1,1,tsp_irq_index);
	msleep(20);
...
static void dowload_fw_work(struct gsl_ts *ts)
{
...
static void gsl_ts_async_work(struct work_struct *work)
{
	struct gsl_ts *ts = container_of(work, struct gsl_ts, rw_async);
  	printk("I'am in gsl_ts_async_work\n");
...
static int __init gsl_ts_init(void)
{
	...
	{
		if(item_equal("ts.model", "gslX680E_COB_l", 0))
		{
