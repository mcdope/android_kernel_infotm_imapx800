...
static uint8_t config_info_M810I[]=      //Do not change the array name
    {
	...
	0x0B,0x03,0x88,0x10,0x10,0x1D,0x00,0x00,0x09,0x00,
	0x00,0x02,0x45,0x30,0x14,0x03,0x00,0x05,0x00,0x03,
	0x00,0x04,0x00,0x33,0x33,0x30,0x30,0x08,0x00,0x25,
	0x22,0x05,0x14,0x10,0x01,0xFE,0x00,0x00,0x00,0x00,
	0x00,0x00,0x00,0x30,0x30,0x20,0x00,0x00,0x00,0x00,
	0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x01

    };

static uint8_t config_info_def[]=      //Do not change the array name
    {
        0x0F,0x80,/*config address*/
	...
	0x19,0x02,0x14,0x10,0x03,0xD4,0x00,0x00,0x00,0x00,
...
//					else if( item_equal("board.name", "M801I", 0)== 1)
//					{
//					}
			else
			{
            ...
            if(item_exist("board.name"))
						{
								if( item_equal("board.name", "M801I", 0)== 1)
								{                
				            if(!((X_value>=718 && X_value<=768)&&(Y_value>=0 && Y_value<=120)))
				            {
				            		 if(!((X_value>=20 && X_value<=60)&&(Y_value>=60 && Y_value<=120)))
							           {
									            if(X_value>=5 && X_value<=10)
																	X_value += 7;
															else if(X_value>=11 && X_value<=15)
																	X_value += 15;
															else if(X_value>=16 && X_value<=30)
																	X_value += 25;
															else if(X_value>=31 && X_value<=50)
																	X_value += 10;
																	
															else if(X_value>=758 && X_value<=763)
																	X_value -= 7;
															else if(X_value>=753 && X_value<=757)
																	X_value -= 15;
															else if(X_value>=738 && X_value<=752)
																	X_value -= 25;
															else if(X_value>=718 && X_value<=737)
																	X_value -= 10;
												
															 if(Y_value>=5 && Y_value<=10)
																	Y_value += 7;
															else if(Y_value>=11 && Y_value<=15)
																	Y_value += 15;
															else if(Y_value>=16 && Y_value<=30)
																	Y_value += 25;
															else if(Y_value>=31 && Y_value<=50)
																	Y_value += 10;
																	
															else if(Y_value>=1014 && Y_value<=1019)
																	Y_value -= 7;
															else if(Y_value>=1009 && Y_value<=1013)
																	Y_value -= 15;
															else if(Y_value>=994 && Y_value<=1008)
																	Y_value -= 25;
															else if(Y_value>=974 && Y_value<=993)
																	Y_value -= 10;
														}						
										}
								}
...
static int __init goodix_ts_init(void)
{
	...
	{
		if(item_equal("ts.model", "gt813", 0))
		{
