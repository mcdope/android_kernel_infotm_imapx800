...
struct i2c_client * i2c_connect_client_gt801 = NULL;
...
/*FC0*/ 0x4C,0x03,0x01,0x05,0x00,TOUCH_MAX_WIDTH_GT801>>8,TOUCH_MAX_WIDTH_GT801&0xff,TOUCH_MAX_HEIGHT_GT801>>8,
        TOUCH_MAX_HEIGHT_GT801&0xff,0x5A,0x5A,0x46,0x46,0x00,0x00,0x05,
...
/*FE0*/ 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
        ...
        0x09,0x33,0x0F,0x03,0x80,0x88,0x90,0x14,
        0x00,0x00,0x00,0x00,0x00,0x06,0x48,0x34,
        0x01,0x03,0x00,0x05,0x00,0x14,0x00,0x20,
        0x00,0x5A,0x5A,0x46,0x46,0x00,0x00,0x13,
        0x19,0x05,0x14,0x10,0x00,0x04,0x00,0x4F,
        0x70,0x78,0x80,0x00,0x32,0x28,0x30,0x20,
        0x00,0x00,0x00,0x00,0x0D,0x40,0x40,0x00,
        0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x01
#endif        
	...
	0x0F,0x03,0x10,0x10,0x10,0x1C,0x00,0x00,0x04,0x00,
	0x00,0x02,0x48,0x32,0x10,0x03,0x00,0x05,0x00,0x03,
	0x00,0x04,0x00,0x62,0x56,0x5C,0x51,0x06,0x00,0x23,
	0x19,0x13,0x14,0x10,0x03,0xD4,0x00,0x00,0x00,0x00,
    ...
    TOUCH_MAX_HEIGHT_GT801  = ((config_info[RESOLUTION_LOC] << 8)\
                        |config_info[RESOLUTION_LOC + 1]);
    TOUCH_MAX_WIDTH_GT801 = ((config_info[RESOLUTION_LOC + 2] << 8)\
                        |config_info[RESOLUTION_LOC + 3]);

    DEBUG_MSG("TOUCH_MAX_WIDTH_GT801  : 0x%d\n", (int)TOUCH_MAX_WIDTH_GT801);
    DEBUG_MSG("TOUCH_MAX_HEIGHT_GT801 : 0x%d\n", (int)TOUCH_MAX_HEIGHT_GT801);
#endif
#if 0
...
{
    s32 ret = -1;
    u8 rd_cfg_buf[100] = {GTP_REG_XMAX_H,};
    u8 i,readtemp[100];
/*
    uint8_t config_info[]=      //Do not change the array name
    {
...
*/
    ...
    printk("SENSOR ID:%d", rd_cfg_buf[1]);
...
#endif
	printk("\nwr_cfg_buf:%x len:%d",*wr_cfg_buf,cfg_info_len[rd_cfg_buf[1]]);
	for(i=0;i<cfg_info_len[rd_cfg_buf[1]];i++){
                if(i%15==0)printk("\n");
		printk("%x ",wr_cfg_buf[i]);
    ...
    while(retry++ < 3)
    {
        ...
        if (ret == 1)
        {
            printk("\nGTP send config successfully!");
            break;
        }
        printk("GTP send config failed,retry.");
    }
        
        printk("read2:\n");
        readtemp[0]=GTP_REG_XMAX_H;
    GTP_DEBUG_FUNC();
    ret=gtp_i2c_read(ts->client, readtemp, 95);
    if(ret > 0){
        for(i=0;i<95;i++){
                if(i%15==0)printk("\n");
                printk("%2x ",readtemp[i]);
        }
    }
...
/*
    while(retry++ < 3)
    {
	    ret=gtp_i2c_write(ts->client,config_info, (sizeof(config_info)/sizeof(config_info[0])));
...
*/
    ...
    printk("\n######## X_MAX = %d,Y_MAX = %d,MAX_TOUCH = %d,TRIGGER=%d",
...
int touch_num_gt801(uint8_t value, int max)
{
    int tmp = 0;
...
/**///DEBUG_COOR("\n");
    ...
    else
    {
        ...
        {
            input_report_key(ts->input_dev, touch_key_array_gt801[count], !!(key&(0x01<<count)));    
        }
    }
    last_key = key;
#endif
...
//    if ((ts->int_trigger_type> 1) && (GTP_GPIO_READ(GTP_INT_PORT) != (ts->int_trigger_type&0x01)))
   if ((ts->int_trigger_type> 1)  != (ts->int_trigger_type&0x01))
    {
	printk("### exit_work_func ###\n");
        goto exit_work_func;
    }
    ...
    finger_current =  (point_data[3-GTP_READ_COOR_ADDR]<<8)+point_data[2-GTP_READ_COOR_ADDR];
    ...
    {
        input_report_key(ts->input_dev, touch_key_array_gt801[count], key_value&(0x01<<count));	
    }   
#endif
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    printk(KERN_INFO "##########[CTP-GT801]-goodix_ts_probe\n");
...
//    writel(0,PAD_SYSM_VA);
    ...
    {
        input_set_capability(ts->input_dev,EV_KEY,touch_key_array_gt801[retry]);
    }
#endif
    ...
    input_set_abs_params(ts->input_dev, ABS_MT_POSITION_X, 0, TOUCH_MAX_HEIGHT_GT801, 0, 0);
    input_set_abs_params(ts->input_dev, ABS_MT_POSITION_Y, 0, TOUCH_MAX_WIDTH_GT801, 0, 0);
#else
    input_set_abs_params(ts->input_dev, ABS_X, 0, TOUCH_MAX_HEIGHT_GT801, 0, 0);
    input_set_abs_params(ts->input_dev, ABS_Y, 0, TOUCH_MAX_WIDTH_GT801, 0, 0);
...
static int __init goodix_ts_init(void)
{
    ...
    printk(KERN_INFO "[CTP-GT801]-Enter %s\n", __func__);

   	if(item_exist("ts.model"))
	{
		if(item_equal("ts.model", "gt801", 0))
		{
    			goodix_wq = create_workqueue("goodix_wq");        //create a work queue and worker thread
    			if (!goodix_wq)
    			{
        			printk(KERN_ERR "[CTP-GT801]-creat workqueue faiked\n");
        			return -ENOMEM;
    			}
    			...
    			printk(KERN_INFO "[CTP-GT801]-i2c_add_driver return %d\n",ret);
    			return ret;
		}
		else
