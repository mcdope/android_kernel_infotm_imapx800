...
#include "mg8697s_i7a3.h"
#include "mg8697s_update_i7a3.h"
...
#define MG_I2C_NAME	"mg8697sI7A3"
#define BUF_SIZE 		30
...
static int ReadFWCheckSum()
{
	...
	unsigned char *p_data = mg8697s_i7a3_firmware;
	#ifdef RANDY_DEBUG    
...
static int UpdateFW_2()
{
	...
	int DataLen = sizeof(mg8697s_i7a3_firmware);
	unsigned char *p_data = mg8697s_i7a3_firmware;
...
//	    read_buf[i] = read_buf[i+1]; 	
  ...
  // Button
	if(read_buf[MG_MODE] == 2)
	{
		...
		{
			case MG_ANDROID_HOME:
				input_event(mg->dev, EV_KEY, KEY_HOME, !!read_buf[TOUCH_KEY_CODE]);
				prev_key = KEY_HOME;
				break;
				
			case MG_ANDROID_BACK:
				input_event(mg->dev, EV_KEY, KEY_BACK, !!read_buf[TOUCH_KEY_CODE]);
				prev_key = KEY_BACK;
				break;
				
			case MG_ANDROID_MENU:
				input_event(mg->dev, EV_KEY, KEY_MENU, !!read_buf[TOUCH_KEY_CODE]);
				prev_key = KEY_MENU;
				break;
				
			case MG_ANDROID_REL:
				input_event(mg->dev, EV_KEY, prev_key, !!read_buf[TOUCH_KEY_CODE]);
				prev_key = KEY_VOLUMEUP;
				break;
				
			case MG_ANDROID_VOL_UP:
				input_event(mg->dev, EV_KEY, KEY_VOLUMEUP, !!read_buf[TOUCH_KEY_CODE]);
				prev_key = KEY_VOLUMEDOWN;
				break;
				
			case MG_ANDROID_VOL_DOWN:
				input_event(mg->dev, EV_KEY, KEY_VOLUMEDOWN, !!read_buf[TOUCH_KEY_CODE]);
				prev_key = KEY_SEND;
				break;
				
			case MG_ANDROID_CALL:
				input_event(mg->dev, EV_KEY, KEY_SEND, !!read_buf[TOUCH_KEY_CODE]);
				break;
				
			default:
				printk("Error Key Code!\n");		
				break;
		}
	}
	// Position
...
//	init_completion(&mg_touch_event);
    ...
    imapx_pad_irq_config(index, INTTYPE_BOTH, FILTER_MAX);
    _sui_irq_num = imapx_pad_irq_number(index);
    err =  request_irq(_sui_irq_num, mg_irq, IRQF_SHARED, client->name, mg);
	if (err < 0) {
		pr_info( "mg8697i7a3_probe: request irq failed\n");
    ...
    //if(item_exist("ts.downloader")){
        ...
        else
        {
            ...
            if(strcmp(downloader_name, "Mg8697i7a3") == 0){	       
    ...
    printk(KERN_INFO "MG8697I7A END Probe++++\n");
	return 0;
exit_set_irq_mode:
...
//	struct mg_data *ts = i2c_get_clientdata(touch_i2c_client);
	printk(KERN_ERR "mg_suspend!\n");
    disable_irq(tsp_irq_index);
/*	
...
static int mg_resume(struct i2c_client *client)
{
	//struct mg_data *ts = i2c_get_clientdata(touch_i2c_client);
   int ret;
   printk(KERN_ERR "mg_resume\n");
   imapx_pad_set_mode(0, 1, tsp_irq_index);
   imapx_pad_irq_config(tsp_irq_index, INTTYPE_BOTH, FILTER_MAX);
   ctp_reset();
   enable_irq(tsp_irq_index);
   /*
...
static int __init mg_init(void)
{
    ...
    printk("------mg8697s_i7a3 module init------\n");
    if(item_exist("ts.model"))
    {
        if(item_equal("ts.model", "Mg8697i7a3", 0))
        {
	...
	return ret;
		...
		printk("%s: touchscreen is not mg8697s_i7a3\n", __func__);
    }
	else
