...
Output:
	None.
*******************************************************/
void gtp_wakeup_green(struct goodix_ts_data *ts,u8 disable)
{
    //GTP_DEBUG_FUNC();
...
polling_begin:
    if ((ts->int_trigger_type> 1) && (GTP_GPIO_READ(ts->irq_pad_index) != (ts->int_trigger_type&0x01)))
    {
        goto exit_work_func;
    }
...
static irqreturn_t goodix_ts_irq_handler(int irq, void *dev_id)
{
    ...
    //GTP_DEBUG_FUNC();
	///leo add
    if(imapx_pad_irq_pending(ts->irq_pad_index))
	    imapx_pad_irq_clear(ts->irq_pad_index);
    else
	    return IRQ_HANDLED;
...
static s8 gtp_wakeup_sleep(struct goodix_ts_data * ts)
{
    ...
    while(retry++<10)
    {
        GTP_GPIO_OUTPUT(ts->irq_pad_index, 0);
        msleep(20);
        GTP_GPIO_AS_INT(ts->irq_pad_index);
        msleep(200);
...
//    const u8 irq_table[] = GTP_IRQ_TAB;
    ...
    ret  = request_irq(ts->client->irq, 
                       ...
                       /*irq_table[ts->int_trigger_type]*/ 0,
                       ts->client->name,
                       ts);
    if (ret)
    {
...
static int goodix_ts_remove(struct i2c_client *client)
{
    ...
    if (ts) 
    {
        if (ts->use_irq)
        {
            GTP_GPIO_AS_INPUT(ts->irq_pad_index);
            GTP_GPIO_FREE(ts->irq_pad_index);
...
static void goodix_ts_early_suspend(struct early_suspend *h)
{
	...
	printk("goodix_ts_early_suspend+++++++++++++++++++++++++++++++++start\n");
	...
	printk("goodix_ts_early_suspend+++++++++++++++++++++++++++++++++end\n");
}
...
static void goodix_ts_late_resume(struct early_suspend *h)
{
    ...
    s8 ret = -1;
    ts = container_of(h, struct goodix_ts_data, early_suspend);
	printk("goodix_ts_late_resume+++++++++++++++++++++++++++++++++start\n");
	...
	printk("goodix_ts_late_resume+++++++++++++++++++++++++++++++++end\n");
}
#endif
...
static void gt81x_reset_detect(void)
{
	int index;
	index = item_integer("ts.reset", 1);
...
static gt81x_start_ping_i2c_addr()
{	
	struct i2c_adapter *adapter=NULL;
	__u16 i2cAddr=0x55;//slave addr
...
if(item_exist("ts.model"))
{
	if(item_equal("ts.model", "gt8105", 0) || DoI2cScanDetectDev())
	{
		...
		printk("%s: touchscreen is not gt8105\n", __func__);
}
else
