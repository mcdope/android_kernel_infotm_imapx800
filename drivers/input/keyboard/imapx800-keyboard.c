...
struct imapx800_keybd{
	struct imapx800_keybd_platform_data *pdata;
...
static void imapx800_keybd_build_keycode(struct imapx800_keybd *keypad)
{
	struct imapx800_keybd_platform_data *pdata = keypad->pdata;
...
static inline unsigned int lookup_matrix_keycode(
		struct imapx800_keybd *keypad, int row, int col)
{
	return keypad->matrix_keycodes[(row << 3) + col];
}

static void imapx800_keybd_scan_matrix(struct imapx800_keybd *keypad)
{
	struct imapx800_keybd_platform_data *pdata = keypad->pdata;
	int iRow, iCol;
...
static irqreturn_t imapx800_keybd_irq_handler(int irq, void *dev_id)
{
	struct imapx800_keybd *keypad = dev_id;
	...
	imapx800_keybd_scan_matrix(keypad);
...
static int imapx800_keybd_open(struct input_dev *dev)
{
	struct imapx800_keybd *keypad = input_get_drvdata(dev);
...
static void imapx800_keybd_close(struct input_dev *dev)
{
	struct imapx800_keybd *keypad = input_get_drvdata(dev);
...
//#ifdef CONFIG_PM
//#else
#define imapx800_keybd_suspend	NULL
#define imapx800_keybd_resume	NULL
//#endif

#define res_size(res)	((res)->end - (res)->start + 1)

static int __devinit imapx800_keybd_probe(struct platform_device *pdev)
{
	struct imapx800_keybd *keypad;
	struct input_dev *input_dev;
	struct resource *res;
	int irq, error;

	//keyboard enable/disable
    module_power_on(SYSMGR_KEYBD_BASE);

    imapx_pad_cfg(IMAPX_KB, 0);
    if (!pdev) {
        dev_err(&pdev->dev, "No platform data\n");
        return -ENODEV;
    }
    /* TODO set caplock and numlock gpio to control the led*/
	pdev->dev.platform_data = &imapx800_keybd_info;

	keypad = kzalloc(sizeof(struct imapx800_keybd), GFP_KERNEL);
	if (keypad == NULL) {
	...
	if (keypad->mmio_base == NULL) {
        ...
        keypad->clk = clk_get(&pdev->dev, "keyboard");
	...
	input_dev->open = imapx800_keybd_open;
	input_dev->close = imapx800_keybd_close;
	...
	imapx800_keybd_build_keycode(keypad);

	platform_set_drvdata(pdev, keypad);

	error = request_irq(irq, imapx800_keybd_irq_handler, IRQF_DISABLED,
			    pdev->name, keypad);
	if (error) {
...
static int __devexit imapx800_keybd_remove(struct platform_device *pdev)
{
	struct imapx800_keybd *keypad = platform_get_drvdata(pdev);
	struct resource *res;
...
MODULE_ALIAS("platform:imapx800_keypad");

static struct platform_driver imapx800_keybd_driver = {
	.probe		= imapx800_keybd_probe,
	.remove		= __devexit_p(imapx800_keybd_remove),
	.suspend	= imapx800_keybd_suspend,
	.resume		= imapx800_keybd_resume,
	.driver		= {
		.name	= "imap-keybd",
		.owner	= THIS_MODULE,
	},
...
static int __init imapx800_keybd_init(void)
{
	return platform_driver_register(&imapx800_keybd_driver);
}

static void __exit imapx800_keybd_exit(void)
{
	platform_driver_unregister(&imapx800_keybd_driver);
}

module_init(imapx800_keybd_init);
module_exit(imapx800_keybd_exit);

MODULE_DESCRIPTION("imapx800 keybd Controller Driver");
MODULE_LICENSE("GPL");
