...
struct workqueue_struct *slpsync_work_queue;
...
static int __init wakelocks_init(void)
{
	...
	wake_lock_init(&sync_wake_lock, WAKE_LOCK_SUSPEND, "sus_sync");
	...
	slpsync_work_queue = create_singlethread_workqueue("suspend_sync");
	if (slpsync_work_queue == NULL) {
		ret = -ENOMEM;
		goto err_suspend_work_queue;
	}
...
static void  __exit wakelocks_exit(void)
{
	...
	destroy_workqueue(slpsync_work_queue);
	platform_driver_unregister(&power_driver);
	platform_device_unregister(&power_device);
	wake_lock_destroy(&suspend_backoff_lock);
	wake_lock_destroy(&unknown_wakeup);
